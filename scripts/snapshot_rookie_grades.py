...
def _run_snapshot(year: int) -> int:
    ...
    for r in rows:
        ...
        hist_id = existing_by_norm.get(_norm(name)) or f"HIST_{year}_{_slug(name)}"
        ...
        payload = {
            ...
            "production_score":      round(float(r.get("production_score") or 0), 2),
            "efficiency_score":      round(float(r.get("efficiency_score") or 0), 2),
            "age_score":             round(float(r.get("age_score") or 0), 2),
            "breakout_profile_score":round(float(r.get("breakout_profile_score") or 0), 2),
            "athleticism_score":     round(float(r.get("athleticism_score") or 0), 2),
            "competition_score":     round(float(r.get("competition_score") or 0), 2),
            "draft_capital_score":   round(float(r.get("projected_draft_capital_score") or 0), 2),
            ...
            if hist_id in existing_ids:
                conn.execute(
                    ...
                    UPDATE historical_prospect_grades SET
                        prospect_score=%(prospect_score)s, tier=%(tier)s, tier_label=%(tier_label)s,
                        overall_rank=%(overall_rank)s, position_rank=%(position_rank)s,
                        production_score=%(production_score)s, efficiency_score=%(efficiency_score)s,
                        age_score=%(age_score)s, breakout_profile_score=%(breakout_profile_score)s,
                        athleticism_score=%(athleticism_score)s, competition_score=%(competition_score)s,
                        draft_capital_score=%(draft_capital_score)s, confidence_score=%(confidence_score)s,
                        actual_pick=%(actual_pick)s, actual_round=%(actual_round)s,
                        actual_nfl_team=%(actual_nfl_team)s, headshot_url=%(headshot_url)s
                ...
                conn.execute(
                    ...
                    ON CONFLICT (player_id) DO UPDATE SET
                        prospect_score=EXCLUDED.prospect_score, tier=EXCLUDED.tier,
                        tier_label=EXCLUDED.tier_label, overall_rank=EXCLUDED.overall_rank,
                        ...
                        actual_pick=EXCLUDED.actual_pick, actual_round=EXCLUDED.actual_round,
