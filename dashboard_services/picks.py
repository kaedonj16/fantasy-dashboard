...
def _load_fc_slot_pick_values(current_year: int, *, is_sf: bool = False) -> Dict[str, float]:
    """
    ...
    Uses the SF FantasyCalc dump when ``is_sf`` so SF boards aren't stuck on
    1QB pick prices.
    """
    try:
        import pandas as pd
        ...
        fc_path = (
            DATA_DIR / "fantasycalc_sf_api_values.csv"
            if is_sf
            else DATA_DIR / "fantasycalc_api_values.csv"
        )
        # Fall back to the 1QB dump if the SF file is missing.
        if is_sf and not fc_path.exists():
        ...
        model_data = _json.loads(model_path.read_text(encoding="utf-8"))

        value_key = "sf_value" if is_sf else "value"
        model_players = [
            p for p in model_data
            if str(p.get("position", "")).upper() not in ("PICK", "")
        ]
        if not model_players:
            return {}
        model_top = max(
            float(p.get(value_key) or p.get("value") or 0) for p in model_players
        )
        if model_top <= 0:
            return {}
...
def _finalize_wls_axis(
    wls_axis: dict,
    *,
    current_year: int,
    is_sf: bool,
) -> Dict[str, float]:
    """Normalize one WLS axis (1qb or sf) into the model pick-value table."""
    _MAX_ROUND = 5
    wls_final: Dict[str, float] = {}
    for key, val in (wls_axis or {}).items():
    ...
    for _group_key, slots in slot_groups.items():
        slots.sort(key=lambda x: x[0])
        running_max = float("inf")
        for _slot_num, key, val in slots:
            capped = min(val, running_max)
            fixed[key] = round(capped, 1)
            running_max = capped
    fixed.update(bucket_entries)

    # Override current-year slot picks with FC-normalized values.
    fc_slots = _load_fc_slot_pick_values(current_year, is_sf=is_sf)
    if fc_slots:
        ...
        for key, val in list(fixed.items()):
            ...
            try:
                ...
                logger.debug("suppressed exception", exc_info=True)
        for _gk, sl in slot_groups2.items():
    ...
    _model_val_key = "sf_value" if is_sf else "value"
    try:
        _mp = DATA_DIR / "model_values.json"
        if _mp.exists():
            _mdata = json.loads(_mp.read_text(encoding="utf-8"))
            for _mp_entry in _mdata:
                ...
                if _myr <= current_year:
                    continue
                _mv = float(
                    _mp_entry.get(_model_val_key) or _mp_entry.get("value") or 0
                )
                if _mv > 0:
                    fixed[_mid] = _mv
    except Exception:
...
def load_pick_value_table(
    ...
    **_kwargs,
    ...
    When ``is_sf`` is True, uses the WLS/FantasyCalc SF axis so SF trade and
    draft boards don't price picks with 1QB capital.
    ...
    if not wls_path.exists():
        candidates = sorted(
            glob.glob(str(DATA_DIR / "pick_values_wls_*.json")), reverse=True
        )
    ...
    if wls_path.exists():
        try:
            wls_data = json.loads(
                wls_path.read_text(encoding="utf-8")
                if hasattr(wls_path, "read_text")
                else open(wls_path, encoding="utf-8").read()
            )
            # Prefer the requested axis; fall back to 1qb if SF is empty.
            wls_axis = wls_data.get(axis) or {}
            if is_sf and not wls_axis:
                wls_axis = wls_data.get("1qb", {})
            fixed = _finalize_wls_axis(
                wls_axis, current_year=current_year, is_sf=is_sf
            )
            if fixed:
                return fixed
        except Exception:
            logger.warning(
                "picks: failed to build pick table from market calibration file",
                exc_info=True,
            )
    ...
    if not final:
        try:
            ...
            value_col = (
                ...
                if is_sf
                else "weighted_market_value_1qb"
            )
            with get_conn() as conn:
                rows = conn.execute(f"""
                    SELECT pick_season, pick_round, pick_bucket,
                           {value_col} AS mkt_val
            ...
            for r in rows:
                ...
                final[key] = float(r["mkt_val"])
        except Exception:
            logger.warning(
                "picks: failed to load pick values from trade_intel_pick_stats",
                exc_info=True,
            )

    # Apply normalization scale for the matching axis
    try:
        _scale_path = DATA_DIR / "market_calibration_scale.json"
        if _scale_path.exists():
            _scale_data = json.loads(_scale_path.read_text(encoding="utf-8"))
            _scale_key = "scale_sf" if is_sf else "scale_1qb"
            _scale = float(
                _scale_data.get(_scale_key)
                or _scale_data.get("scale_1qb", 1.0)
            )
