...
def build_usage_map_for_season(
    ...
    for week, players in season_stats.items():
        ...
        for pid, row in players.items():
            # In your real payload, `row` *is* the stats dict
            if not isinstance(row, dict):
                continue

            stats = row

            # Safely parse numeric fields (using Sleeper's actual keys)
            ...
            rush_yards = float(stats.get("rush_yd", stats.get("rushing_yd", 0)) or 0)
    ...
    for pid, acc in accum.items():
        ...
        usage[pid] = {
            ...
            "std_ppg": 0.0,  # fill in later if you want stdev
        }
    return usage
