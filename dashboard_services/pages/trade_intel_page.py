"""Trade Intelligence page body, shared by the standalone /trade-intel page
and the Trade Hub's Market Intel tab.

The markup, styles, and script are one self-contained unit (ti- prefixed
classes, an IIFE, global onclick helpers). It renders in exactly one place
per page, so embedding it in the hub tab needs no ID namespacing.
"""
...
def build_trade_intel_body(
    *,
    ...
    """Render the Trade Intelligence UI.

    ``embedded=True`` strips the outer card wrapper and page header so the
    hub tab can host the same controls/grid/modal/script without a
    card-in-card. PRO gating stays server-side: the /api/trade-intel/*
    endpoints 403 for non-PRO, and the caller renders the paywall teaser
    for non-PRO users.
    """
    _has_premium_str = "true" if has_premium else "false"
    ...
    {open_html}
        ...
        <div class="ti-controls">
          ...
          <div class="otc-day-filters ti-lf-bar" style="margin:0;" id="tiLeagueTypeBar">
            <button class="otc-day-filter ti-lf-btn {'active' if league_type != 'sf' else ''}" data-lf="1qb" onclick="switchTILeagueType('1qb')">1QB</button>
            <button class="otc-day-filter ti-lf-btn {'active' if league_type == 'sf' else ''}" data-lf="sf"  onclick="switchTILeagueType('sf')">SF</button>
          </div>
        </div>
    ...
    <style>
      ...
      /* Shared "why this" ranking-explanation line (trade hub component). */
      .th-why {{
        display: flex;
        gap: 6px;
    ...
    (function() {{
      ...
      const TI_HAS_PREMIUM = {_has_premium_str};
      ...
      let TI_LEAGUE_TYPE = '{league_type}';
      const TI_LEAGUE_SIZE = {league_size};
      let currentPage = 1;
