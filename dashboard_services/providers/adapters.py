...
class SleeperProvider(ProviderAdapter):
    ...
    def get_matchups(self, league_id, season, week, *, cache_ttl=None):
        ...
        kwargs = {"_cache_max_age": cache_ttl} if cache_ttl is not None else {}
        return get_matchups(league_id, week, **kwargs)
...
class ESPNProvider(ProviderAdapter):
    ...
    def get_matchups(self, league_id, season, week, *, cache_ttl=None):
        return self._api().get_matchups(
            season, league_id, week, cache_ttl=cache_ttl,
        )
...
class YahooProvider(ProviderAdapter):
    ...
    def get_matchups(self, league_id, season, week, *, cache_ttl=None):
        from . import yahoo_api
        from dashboard_services.platform_api import _yahoo_token
        return yahoo_api.get_matchups(
            season, league_id, week, _yahoo_token(league_id, season),
            cache_ttl=cache_ttl,
        )
    def get_traded_picks(self, league_id, season): return []
    def get_bracket(self, league_id, season, kind):
