...
_CLOSE_PROBABILITY_MIN = 45
_CLOSE_PROBABILITY_MAX = 55
...
def _narrative_cache_key(platform, league_id, season, target_week, game: dict) -> str:
    """Key prose by stable, material facts; live projections remain uncached HTML."""
    fields = ("matchup_id", "roster_id_a", "roster_id_b", "team_a", "team_b",
              "record_a", "record_b", "rank_a", "rank_b", "streak_a", "streak_b",
              "h2h", "why")
    context = {key: game.get(key) for key in fields}
    for key in ("out_a", "out_b", "maybe_a", "maybe_b", "bye_a", "bye_b"):
        context[key] = [(p.get("name"), p.get("status")) for p in game.get(key) or []]
    digest = hashlib.sha256(json.dumps(context, sort_keys=True, default=str).encode()).hexdigest()[:16]
    return (f"weekly_gotw_narrative_{str(platform).lower()}_{league_id}_{season}_"
            f"w{target_week}_v3_{digest}")


def _fallback_preview(game: dict) -> str:
    """Build restrained, factual prose without treating absent projections as tied."""
    a, b = str(game.get("team_a") or "Team A"), str(game.get("team_b") or "Team B")
    sentences = [f"{a} and {b} meet in a matchup with a real reason to watch."]
    wp = game.get("win_prob_a")
    if isinstance(wp, (int, float)) and _CLOSE_PROBABILITY_MIN <= wp <= _CLOSE_PROBABILITY_MAX:
        sentences.append("The projections have it close enough that either lineup could swing it.")
    elif (game.get("h2h") or {}).get("meetings"):
        sentences.append("They have already met this season, so this is a chance to revisit that result.")
    elif game.get("streak_a", "").startswith("W") or game.get("streak_b", "").startswith("W"):
        hot = a if str(game.get("streak_a", "")).startswith("W") else b
        sentences.append(f"{hot} comes in with some recent momentum.")
    concerns = [p.get("name") for side in ("out_a", "out_b", "maybe_a", "maybe_b")
                for p in game.get(side) or [] if p.get("name")]
    if concerns:
        sentences.append(f"Keep an eye on {concerns[0]}'s availability before lineups lock.")
...
def _reason_bits(
    ...
    if top_factor == "closeness":
        pct_a = round(win_prob * 100)
        if _CLOSE_PROBABILITY_MIN <= pct_a <= _CLOSE_PROBABILITY_MAX:
            return "Dead heat on paper"
        return "Projections add an interesting angle"
...
def _build_next_week_preview(
    ...
    for m in matchups:
        ...
        phrase = {
            ...
            "closeness": "projects as a coin flip" if have_proj and
                         _CLOSE_PROBABILITY_MIN <= round(win_prob * 100) <= _CLOSE_PROBABILITY_MAX else None,
...
def _generate_ai_storyline(payload: dict) -> dict:
    ...
    schema = {
        ...
        "properties": {
            ...
            "looking_ahead": {
                "type": "string",
                "description": "One coherent 3-4 sentence spoken-style preview of the selected game. Empty if no preview data is given.",
            },
        },
...
For "looking_ahead": if next_week_preview is null, return an empty string. Otherwise write one coherent paragraph of 3-4 concise, conversational sentences that discusses exactly the selected teams. Lead with their actual storyline, explain why it is interesting using only supplied facts, and close naturally. Format records naturally as 1-0. Use % rather than the word percent, but do not repeat projections or win percentages because the card shows them. Never invent a rivalry, injury, stakes, performance, or prior result. In Weeks 1-2 keep claims proportionate to the small sample. Do not call it "the most interesting game", "the marquee matchup", a "heavyweight clash", or "heavyweight test".
""".strip()
...
def get_weekly_ai_recap(
    ...
    if (not recap_html or (game and not looking_ahead)) and ai_available():
        try:
            ...
            generated = str(result.get("looking_ahead") or "").strip()
            if game and generated:
                looking_ahead = generated
                save_cached_ai_text(narrative_key, looking_ahead)
    ...
    if preview and game:
        if not looking_ahead:
            looking_ahead = _fallback_preview(game)
