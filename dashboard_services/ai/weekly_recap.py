...
from dashboard_services.recap import (
    apply_ai_narrative,
...
def get_weekly_ai_recap(
    ...
    if cached is not None:
        cached_document = recap_document_from_json(cached)
        # Completed weeks remain cached indefinitely, but a corrected score or
        # changed next-week availability produces a new signature immediately.
        if (cached_document is not None
                and cached_document.get("data_signature") == document["data_signature"]):
            narrative = cached_document["narrative"]
            return (
    ...
    if not ai_available():
        # A recap is useful even without an AI provider.  The document builder
        # supplies restrained, fact-only fallback copy selected from the same
        # deterministic stories that other recap formats will consume.
