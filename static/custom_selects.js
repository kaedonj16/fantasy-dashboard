...
(function () {
    ...
    function initOne(sel) {
        ...
        function getEnabled() {
            return Array.from(list.querySelectorAll('.csd-option:not(.is-disabled)'));
        }
        ...
        function openList() {
            ...
            var opts = getEnabled();
            focIdx = opts.findIndex(function (el) {
                return el.dataset.value === sel.value;
            });
            setFocus(focIdx);
        }

        wrap._csdReposition = function () {
        ...
        function closeList() {
            ...
            var _l = list;
            setTimeout(function () {
                if (!isOpen) _l.style.display = 'none';
            }, 150);
        }
        ...
        trigger.addEventListener('keydown', function (e) {
            ...
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                isOpen ? setFocus(Math.min(focIdx + 1, opts.length - 1)) : openList();
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                isOpen ? setFocus(Math.max(focIdx - 1, 0)) : openList();
            } else if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                isOpen ? (focIdx >= 0 ? pick(opts[focIdx].dataset.value) : closeList()) : openList();
    ...
    document.addEventListener('click', function () {
        if (_openWrap) _openWrap._csdClose();
    });
    ...
    window.addEventListener('scroll', function () {
        if (_openWrap && _openWrap._csdReposition) _openWrap._csdReposition();
    }, true);
