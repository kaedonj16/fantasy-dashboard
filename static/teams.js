...
(function () {
    ...
    async function _applyFcStartupAdp(data) {
        ...
        try {
            ...
            fcData
                .filter(function (e) {
                    return e.overallRank && e.player && e.player.sleeperId;
                })
                .sort(function (a, b) {
            ...
            (data.teams || []).forEach(function (team) {
                ...
                if (!gs.length) return;
                var avg = gs.reduce(function (a, b) {
                    return a + b;
                }, 0) / gs.length;
    ...
    function loadDraft() {
        ...
        fetch('/api/draft-grades?platform=' + _platform +
            ...
            .then(async function (data) {
                ...
                var teams = data.teams || [];
                if (!teams.length) {
                    _panelEmpty(panel, 'No draft data', 'Draft grades will appear once picks are recorded.');
                    return;
                }
                ...
                teams.forEach(function (t) {
                    teamNames[t.roster_id] = t.team_name;
                });
                ...
                function psColor(ps) {
                    return ps >= 90 ? '#22c55e' : ps >= 75 ? '#38bdf8' : ps >= 60 ? '#f59e0b' : '#ef4444';
                }

                // Team-grade color band — matches the Draft Room League tab (tCol).
                function gradeCol(s) {
                    return s >= 75 ? '#22c55e' : s >= 60 ? '#38bdf8' : s >= 45 ? '#f59e0b' : '#ef4444';
                }
                ...
                function buildByRoundHtml(round) {
                    var roundPicks = allPicks.filter(function (p) {
                    ...
                    if (!roundPicks.length) {
                        ...
                        roundPicks.forEach(function (p) {
                            html += renderPickRow(p, true);
                        });
                    }
                    html += '</div>';
                    return html;
                ...
                function renderRoundView(container, round) {
                    ...
                    if (prev) prev.addEventListener('click', function () {
                        if (currentRound > 1) {
                            currentRound--;
                            renderRoundView(container, currentRound);
                        }
                    });
                ...
                panel.querySelectorAll('.draft-view-tab').forEach(function (tab) {
                    tab.addEventListener('click', function () {
                        panel.querySelectorAll('.draft-view-tab').forEach(function (t) {
    ...
    function loadPowerRankings() {
        ...
        fetch('/api/power-rankings', {
            ...
            .then(data => {
                if (!data.success) {
                    _panelEmpty(panel, 'Couldn’t load', data.error || 'Failed to load.', {error: true, icon: 'error'});
                    return;
                }
                if (!data.html) {
            ...
            .catch(function () {
                _panelError(panel, 'Could not load power rankings.');
            });
    }
