(function () {
    ...
    var posColor = function (p) {
        return POS_COLOR[(p || '').toUpperCase()] || '#94a3b8';
    };
    var hsUrl = function (id) {
    ...
    function _posIsAll() {
        for (var _k in posFilter) {
            if (posFilter.hasOwnProperty(_k)) return false;
        }
        return true;
    }

    function _posMatches(pos) {
        return _posIsAll() || !!posFilter[String(pos || '').toUpperCase()];
    }
    ...
    function _syncPosPills() {
        var all = _posIsAll();
        var host = document.getElementById('drPosFilters');
        if (!host) return;
    ...
    function ensureOwned() {
        if (state && !state.owned) state.owned = defaultOwned();
    }

    function ownedPicks() {
        ensureOwned();
        if (!state || !state.owned) return [];
        return Object.keys(state.owned).filter(function (k) {
    ...
    function ownsAnyInColumn(slot) {
        return picksInColumn(slot).some(function (pn) {
            return isMyPick(pn);
        });
    }
    ...
    function ownsAllInColumn(slot) {
        ...
        return pcs.length > 0 && pcs.every(function (pn) {
            return isMyPick(pn);
        });
    }
    ...
    function nextOwnedAfterCurrent() {
        ...
        for (var i = 0; i < ups.length; i++) {
            if (ups[i] > state.current) return ups[i];
        }
        return null;
    }
    ...
    function upcomingOwnedPicks() {
        return ownedPicks().filter(function (pn) {
            return pn >= state.current && !state.picks[pn];
        });
    }
    ...
    function save() {
        try {
            sessionStorage.setItem(sessKey, JSON.stringify(state));
        } catch (e) {
        }
    }
    ...
    function load() {
        try {
            return JSON.parse(sessionStorage.getItem(sessKey) || 'null');
        } catch (e) {
            return null;
        }
    }
    ...
    function cheatDraftedIds() {
        var ids = [];
        Object.keys(drafted).forEach(function (id) {
            if (drafted[id]) ids.push(String(id));
        });
        return ids;
    }
    ...
    function applyCfgDefaults() {
            ...
            try {
                ...
                if (dtBb) {
                    for (var bbi = 0; bbi < dtBb.options.length; bbi++) {
                        if (dtBb.options[bbi].value === 'redraft') {
                            dtBb.selectedIndex = bbi;
                            break;
                        }
                    }
                }
                ...
                if (bbPreset) {
                    ...
                    if (!cfg.rosterPositions) {
                        ...
                        _setupRoster.TAXI = 0;
                        _setupRoster.IDP = 0;
                        _setupRoster._sf = !!bbPreset.SF;
                        _setupRoster._rd = true;
                    }
                }
            } catch (e) {
            }
        ...
        function refreshCheatHrefs() {
            ...
            [_cs, _cs2, _csPop].forEach(function (a) {
        ...
        refreshCheatHrefs();
        [_cs, _cs2, _csPop].forEach(function (a) {
            if (a) a.addEventListener('mousedown', refreshCheatHrefs);
        });
        ...
        if (_csOverlay) _csOverlay.addEventListener('click', function (e) {
            if (e.target === _csOverlay) closeCheatSheet();
        });
        ...
        if (cfg.numTeams) {
            ...
            for (var i = 0; i < t.options.length; i++) {
                if (t.options[i].value === want || t.options[i].text === want) {
                    t.selectedIndex = i;
                    break;
                }
            }
        ...
        if (cfg.scoring) {
            var cfgPpr = document.getElementById('drPpr');
            if (cfgPpr) cfgPpr.value = String(cfg.scoring.ppr != null ? cfg.scoring.ppr : 1);
            var cfgTep = document.getElementById('drTep');
            if (cfgTep) cfgTep.value = String(cfg.scoring.tep != null ? cfg.scoring.tep : 0);
            var cfgPassTd = document.getElementById('drPassTd');
            if (cfgPassTd) cfgPassTd.value = String(cfg.scoring.passTd >= 6 ? 6 : 4);
        }
    ...
    function _reconcileRoster(r, sf, rd) {
        var out = {};
        ['QB', 'SF', 'RB', 'WR', 'TE', 'FLEX', 'RB_WR', 'WR_TE', 'RB_TE', 'K', 'DEF', 'BN', 'IR', 'TAXI', 'IDP'].forEach(function (k) {
            out[k] = r[k] || 0;
        });
        if (sf) {
            if (!out.SF) out.SF = 1;
            if (!out.FLEX) out.FLEX = 1;
        } else {
            out.SF = 0;
        }
    ...
    function renderSetupRoster() {
        ...
        Array.prototype.forEach.call(document.querySelectorAll('[data-roster-preset]'), function (btn) {
            btn.addEventListener('click', function () {
                ...
                _setupRoster = {};
                ['QB', 'SF', 'RB', 'WR', 'TE', 'FLEX', 'K', 'DEF', 'BN'].forEach(function (k) {
                    _setupRoster[k] = preset[k] || 0;
                });
                _setupRoster.IR = 0;
                _setupRoster.TAXI = 0;
                _setupRoster.IDP = 0;
                _setupRoster._sf = !!preset.SF;
                _setupRoster._rd = preset.type === 'redraft';
                renderSetupRoster();
                renderSetupCapital();
            });
        });
    ...
    function _seedOwnedForRandomSlot(st) {
        ...
        _setupOwned = defaultSetupOwned({
            teams: st.teams, rounds: st.rounds,
    ...
    function capSlotPicker(r, c) {
        var owned = {};
        roundPicks(r, c).forEach(function (pn) {
            owned[slotOnClock(pn, c.teams, c.order)] = true;
        });
        var cells = '';
        for (var sl = 1; sl <= c.teams; sl++) {
            var pn = pickNum(r, sl, c.teams, c.order);
    ...
    function capRow(r, c) {
        var pills = roundPicks(r, c).map(function (pn) {
            return capPill(pn, c);
        }).join('');
    ...
    function renderSetupCapital() {
        ...
        var total = Object.keys(_setupOwned).filter(function (k) {
            return _setupOwned[k];
        }).length;
        ...
        if (c.rounds > splitAt) {
            ...
            var lateBody = '';
            if (_capLateOpen) {
                for (var lr2 = splitAt + 1; lr2 <= c.rounds; lr2++) lateBody += capRow(lr2, c);
            }
    ...
    function readScoring() {
        ...
        return {
            ppr: isNaN(ppr) ? 1.0 : ppr, tep: isNaN(tep) ? 0 : tep,
            passTd: passTd >= 6 ? 6 : 4
        };
    }
    ...
    function scoringCfg() {
        ...
        return {
            ppr: s.ppr != null ? s.ppr : 1.0, tep: s.tep != null ? s.tep : 0,
    ...
    function syncCpuAdpSourceOptions(preferred) {
        ...
        var fromPayload = adpSourceOptions && adpSourceOptions[t] && adpSourceOptions[t].length
            ? adpSourceOptions[t].map(function (o) {
                return o.value;
            }) : null;
    ...
    function showMain() {
        ...
        var _kl = document.getElementById('drKeeperList');
        if (_kl) _kl.hidden = true;
        var _kv = document.getElementById('drKeeperView');
        if (_kv) {
            _kv.classList.remove('is-open');
            _kv.setAttribute('aria-expanded', 'false');
        }
    ...
    function hydrateSetupFromState() {
        ...
        var sfEl = document.getElementById('drSf');
        if (sfEl && state.sf != null) sfEl.value = state.sf ? '1' : '0';
        var teamsEl = document.getElementById('drTeams'), wt = String(state.teams);
        if (teamsEl) {
            for (var ti = 0; ti < teamsEl.options.length; ti++) {
                if (teamsEl.options[ti].value === wt || teamsEl.options[ti].text === wt) {
                    teamsEl.selectedIndex = ti;
                    break;
                }
            }
        }
        var ordEl = document.getElementById('drOrder');
        if (ordEl && state.order) ordEl.value = state.order;
        ...
        var rEl = document.getElementById('drRounds');
        if (rEl && state.rounds) rEl.value = String(state.rounds);
        fillSlotOptions(state.teams);   // slot options depend on team count
        var slotEl = document.getElementById('drSlot');
        if (slotEl && state.slot) slotEl.value = String(state.slot);
        if (state.scoring) {
            var pprEl = document.getElementById('drPpr');
            if (pprEl) pprEl.value = String(state.scoring.ppr != null ? state.scoring.ppr : 1);
            var tepEl = document.getElementById('drTep');
            if (tepEl) tepEl.value = String(state.scoring.tep != null ? state.scoring.tep : 0);
            var passTdEl = document.getElementById('drPassTd');
            if (passTdEl) passTdEl.value = String(state.scoring.passTd >= 6 ? 6 : 4);
        }
        if (state.keeper) {
            var kcEl = document.getElementById('drKeeperCount');
            if (kcEl && state.keeperCount != null) {
                kcEl.value = String(state.keeperCount);
                kcEl.dataset.touched = '1';
            }
            var ksEl = document.getElementById('drKeeperSource');
        ...
        if (state.owned) {
            ...
            Object.keys(state.owned).forEach(function (k) {
                if (state.owned[k]) ow[k] = true;
            });
            _setupOwned = ow;
            var _c = setupCtl();
            _setupOwnedSig = [_c.teams, _c.rounds, _c.order, _c.slot].join('|');
        }
    }
    ...
    function applyEditedSetup() {
        ...
        var hasPicks = !!(state.picks && Object.keys(state.picks).some(function (k) {
            return !!state.picks[k];
        }));
    ...
    function isKnownKeeperId(id) {
        ...
        var sid = String(id);
        if (keeperSet && keeperSet.some(function (k) {
            return k && String(k.id) === sid;
        })) return true;
        return false;
    }
    ...
    function resolvedAdp(p, source) {
        ...
        return {
            selectedAdp: selected != null ? selected : (consensus != null ? consensus : adpOf(p)),
    ...
    function consensusAdpOf(p) {
        return adpBySource(p, 'consensus');
    }
    ...
    function loadPlayers() {
            ...
            return fetch(url, {cache: 'no-store'}).then(function (r) {
                return r.text().then(function (body) {
                    ...
                    try {
                        ...
                        throw new Error('Player API returned non-JSON (HTTP ' + r.status + ')');
                    }
                    if (!r.ok) {
                ...
                if (attempt < 1) return new Promise(function (resolve) {
                    setTimeout(function () {
                        resolve(_loadPlayerPayload(attempt + 1));
                    }, 600);
                });
                throw err;
            });
        ...
        _loadPlayerPayload(0)
            .then(function (resp) {
                ...
                if (state.type === 'redraft') {
                    players.slice().sort(function (a, b) {
                        return redraftVal(b) - redraftVal(a);
                    })
                ...
                if (state.mode === 'live' && lastLivePicks) {
                    ...
                    drafted = {};
                    Object.keys(state.picks).forEach(function (k) {
                        var pp = state.picks[k];
                        if (pp) drafted[String(pp.id)] = true;
                    });
                }
            ...
            .catch(function (err) {
                ...
                retry.type = 'button';
                retry.className = 'dr-btn dr-btn-sm';
    ...
    function initKeepers() {
            ...
            if (ovRaw) {
                ...
                if (ov && String(ov.leagueId) === String(cfg.leagueId) && vr) {
                    ...
                    (ov.players || []).forEach(function (p) {
                        if (p && p.id != null) detail[String(p.id)] = p;
                    });
        ...
        var _hasKeeperType = _tEl && Array.prototype.some.call(_tEl.options, function (o) {
            return o.value === 'keeper';
        });
    ...
    function computeKeeperSet() {
        ...
        if (source !== 'assistant' && keeperOverride) {
            ...
            out.forEach(function (k) {
                if (String(k.rosterId) === vr) meta[String(k.id)] = k;
            });
            ...
            var mine = keeperOverride.ids.map(function (id) {
                ...
                if (d) {
                    ...
                    base = {
                        id: String(id), rosterId: vr, projected: false,
    ...
    function keeperSlotMap() {
        ...
        rosters.forEach(function (rid, i) {
            if (i < free.length) map[String(rid)] = free[i];
        });
        return map;
    }
    ...
    function teamCounts(slot) {
        ...
        Object.keys(state.picks).forEach(function (k) {
            if (slotOnClock(parseInt(k, 10), state.teams, state.order) === slot) {
                var pos = (state.picks[k].position || '').toUpperCase();
                if (c[pos] != null) c[pos]++;
            }
        });
        return c;
    ...
    function _simSeed() {
        if (state.simSeed == null) state.simSeed = Math.floor(Math.random() * 2147483647);
        return state.simSeed;
    }
    ...
    function stratLabel(s) {
        return {
            rb_heavy: 'RB heavy', wr_heavy: 'WR heavy', zero_rb: 'Zero RB',
            hero_rb: 'Hero RB', elite_te: 'Elite TE', early_qb: 'Early QB'
        }[s] || '';
    }
    ...
    function _cpuCtx(slot) {
        ...
        for (var pn2 = state.current + 1; pn2 <= tot; pn2++) {
            if (slotOnClock(pn2, state.teams, state.order) === slot) {
                nextOwned = pn2;
                break;
            }
        }
        ...
        return {
            qualByPos: qualByPos, picksList: picksList, rosterQualities: rosterQualities, nextOwned: nextOwned,
    ...
    function _cpuKDefMustFill(pool, counts, remaining) {
        ...
        function bestAt(pos) {
            var cands = pool.filter(function (p) {
                return String(p.position || '').toUpperCase() === pos;
            });
    ...
    function simPick() {
        ...
        var _stSlots = {
            QB: (_rs.QB || 0) + _sfSlots,
            RB: (_rs.RB || 0) + (_rs.FLEX || 0),
            WR: _rs.WR || 0,
            TE: _rs.TE || 0,
            K: _rs.K || 0,
            DEF: _rs.DEF || 0
        ...
        pool.forEach(function (p) {
            ...
            if (pv != null && pv >= 55) {
                var _pp = (p.position || '').toUpperCase();
                posQualLeft[_pp] = (posQualLeft[_pp] || 0) + 1;
            }
        ...
        ['QB', 'RB', 'WR', 'TE'].forEach(function (pos) {
            ...
            cands.filter(function (c) {
                return (c.p.position || '').toUpperCase() === pos && c.pv != null;
            })
                .sort(function (a, b) {
                    return b.pv - a.pv;
        ...
        cands.forEach(function (c) {
            ...
            if (pv != null && window.DraftBoardCore) {
                ...
                c.ds = DraftBoardCore.decisionScore({
                    base: pv, utility: _util,
            ...
            if (!cfg.isBestBall && state.type === 'redraft' && p.bye_week && window.DraftBoardCore && DraftBoardCore.byeWeekSeverity) {
                ...
                (cpuCtx.picksList || []).forEach(function (mp) {
                    ...
                    _cpuBye.push({
                        id: String(f.id || mp.id),
                        pos: bp,
                        bye: f.bye_week != null ? Number(f.bye_week) : null,
                        quality: ppgNormOf(f) || 0
                    });
                });
                ...
                _cpuBye.forEach(function (r) {
                    if (_cby[r.pos]) _cby[r.pos].push(r);
                });
                Object.keys(_cby).forEach(function (bp) {
                    _cby[bp].sort(function (a, b) {
                        return (b.quality || 0) - (a.quality || 0);
                    });
                    _cby[bp].forEach(function (r, i) {
                        r.role = i < (_creq[bp] || 0) ? 'starter' : (i === (_creq[bp] || 0) ? 'primary' : 'fringe');
                    });
                });
                var _week = DraftBoardCore.byeWeekSeverity(_cpuBye, {sf: !!state.sf, tep: scoringCfg().tep})
                    .filter(function (w) {
                        return +w.week === Number(p.bye_week);
                    })[0];
        ...
        if (window.DraftBoardCore && DraftBoardCore.selectDecisionCandidate) {
            var selected = DraftBoardCore.selectDecisionCandidate(
                cands.map(function (c) {
                    return {ref: c, ds: c.ds, weight: c.w};
                }), _curRound, persona, Math.random);
    ...
    function _autoKDefNeed(pool) {
        ...
        var remaining = ownedPicks().filter(function (pn) {
            return pn >= state.current && !state.picks[pn];
        }).length;
        ...
        var cands = pool.filter(function (p) {
            return want[(p.position || '').toUpperCase()];
        });
        if (!cands.length) return null;
        cands.sort(function (a, b) {
            return lineupScore(b) - lineupScore(a);
    ...
    function _simError(where, e) {
        try {
            console.error('[draft-room] ' + where + ' failed:', e);
        } catch (_e) {
        }
    ...
    function _doAutoPick() {
        ...
        try {
            ...
            if (!ap) {
                _simError('auto pick', new Error('No legal players remain for the open roster spot'));
                endSim();
                render();
                return;
            }
    ...
    function simStep() {
        ...
        if (isMyPick(state.current)) {
            if (simAutoDraft) {
                clearTimeout(simTimer);
                simTimer = setTimeout(_doAutoPick, simSpeed);
                return;
            }
            render();
            return; // your turn - wait for manual pick
        }
        var p;
        try {
            p = simPick();
        ...
        if (!p) {
            _simError('CPU pick', new Error('No legal players remain before the draft is complete'));
            endSim();
            render();
            return;
        }
    ...
    function _resetTransient() {
        ...
        _poServerSig = null;
        _poFetching = false;
        _poFailedSig = null;   // drop cached playoff odds
        _poMcCache = null;
        _poMcSig = null;
    ...
    function syncSimControls() {
        ...
        if (running) {
            tg.textContent = simPaused ? 'Resume' : 'Pause';
        }
        if (ab) {
    ...
    function startPracticeMock() {
        ...
        drConfirm('Start a practice mock using these draft settings? Your live sync will stop until you reconnect.', 'Start Mock', function () {
            ...
            var ownedCopy = {};
            if (prev.owned) {
                Object.keys(prev.owned).forEach(function (k) {
                    if (prev.owned[k]) ownedCopy[k] = true;
                });
            }
            ...
            var nameCopy = {};
            if (prev.slotNames) {
                Object.keys(prev.slotNames).forEach(function (k) {
                    nameCopy[k] = prev.slotNames[k];
                });
            }
            ...
            var ownerCopy = {};
            if (prev.pickOwners) {
                Object.keys(prev.pickOwners).forEach(function (k) {
                    ownerCopy[k] = prev.pickOwners[k];
                });
            }
            _resetTransient();
            state = {
    ...
    function posTargets() {
        ...
        var cap = {
            QB: sf ? 4 : Math.max(1, rs.QB || 0), RB: 7, WR: 7,
    ...
    function myPosCounts() {
        var c = {QB: 0, RB: 0, WR: 0, TE: 0, K: 0, DEF: 0};
        myPicksList().forEach(function (p) {
            var pos = (p.position || '').toUpperCase();
            if (c[pos] != null) c[pos]++;
        });
        return c;
    ...
    function listInto(html) {
        document.getElementById('drBaList').innerHTML = html;
    }
    ...
    function computePpgScale(pool) {
        ...
        Object.keys(byPos).forEach(function (pos) {
            ...
            var eliteSum = 0;
            for (var i = 0; i < topN; i++) eliteSum += arr[i];
    ...
    function refreshPsPool() {
        ...
        pool.forEach(function (p) {
            p._ds = liveDecisionScore(p, counts);
        });
        _psPoolMax = mx;
        return mx;
    }
    ...
    function hasCustomBoardOrder() {
        return players.some(function (p) {
            return p.bucket === -1 || p.bucket === 1 || p.moved;
        });
    }
    ...
    function rankedRecommendationPool() {
        var list = availablePool().filter(function (p) {
            return p._ds != null;
        });
    ...
    function _ensureRelScores() {
        ...
        if (players.length) {
            ...
            var byVal = players.slice().sort(function (a, b) {
                return valOf(b) - valOf(a);
            });
            var order = Object.keys(state.picks).filter(function (k) {
                return state.picks[k];
            })
                .map(function (k) {
            ...
            order.forEach(function (pn) {
                ...
                for (var i = 0; i < byVal.length && cand.length < K; i++) {
                    var q = byVal[i];
                    if (taken[String(q.id)]) continue;
                    if (!cand.length) maxV = valOf(q);
                    cand.push(q);
                }
                if (pkFull && !cand.some(function (c) {
                    return String(c.id) === String(pk.id);
                })) cand.push(pkFull);
                var best = 0, mine = 0;
                for (var j = 0; j < cand.length; j++) {
                    ...
                    if (s != null) {
                        if (s > best) best = s;
                        if (String(cand[j].id) === String(pk.id)) mine = s;
                    }
                }
                if (best > 0 && mine > 0) {
                    var d = Math.round(97 * mine / best);
                    map[pn] = d > 99 ? 99 : (d < 1 ? 1 : d);
                } else if (pk.psRel != null) {
                    map[pn] = pk.psRel;
                }
    ...
    function psCtx() {
        ...
        _psCtxCache = {
            targets: targets, qualByPos: qualByPos, rosterQualities: rosterQualities,
    ...
    function draftModelInvalidate() {
        _draftModelCache = null;
    }
    ...
    function observedDraftModel() {
        ...
        if (totalMade >= teams) {
            ...
            made.forEach(function (m) {
                overall[m.pos] = (overall[m.pos] || 0) + 1;
            });
            win.forEach(function (m) {
                recent[m.pos] = (recent[m.pos] || 0) + 1;
            });
    ...
    function availColor(pct) {
        return pct >= 65 ? '#22c55e' : pct >= 40 ? '#f59e0b' : '#ef4444';
    }
        ...
        pool.forEach(function (p) {
            ...
            if (!byPos[pos]) {
                ...
                if (a != null && ab != null) {
                    ...
                    byPos[pos] = p;
                } else if (a == null && ab == null && valOf(p) > valOf(byPos[pos])) {
                    byPos[pos] = p;
                }
            }
        });
    ...
    function myByeSnapshot(extra) {
        ...
        var byPos = {QB: [], RB: [], WR: [], TE: []};
        rows.forEach(function (r) {
            if (byPos[r.pos]) byPos[r.pos].push(r);
        });
        Object.keys(byPos).forEach(function (pos) {
            byPos[pos].sort(function (a, b) {
                return (b.quality || 0) - (a.quality || 0);
        ...
        if (req.FLEX > 0) {
            ...
            ['RB', 'WR', 'TE'].forEach(function (pos) {
                byPos[pos].slice(req[pos] || 0).forEach(function (r) {
                    flexPool.push(r);
                });
            });
    ...
    function byeConflictLevel(p) {
        ...
        var w0 = without[0], w1 = withP.filter(function (w) {
            return +w.week === Number(p.bye_week);
        })[0] || withP[0];
    ...
    function openCompare() {
        ...
        function cmpCol(p, other) {
            ...
            return '<div class="dr-cmp-player">'
                ...
                + statRow('Value', f.value, o.value, true, function (x) {
                    return x != null ? String(Math.round(x)) : '-';
                })
                + statRow(ppgRowLbl, ppg, oppg, true, function (x) {
                    return x != null ? x.toFixed(1) : 'N/A';
                })
                + (f.lastPpg != null || o.lastPpg != null ? statRow((f.ppgSeason || 'Last') + ' PPG', f.lastPpg, o.lastPpg, true, function (x) {
                    return x != null ? x.toFixed(1) : '-';
                }) : '')
                + statRow('VOR', f.vor, o.vor, true, function (x) {
                    return x != null ? fmtSigned(x, Number.isInteger(x) ? 0 : 1) : '-';
                })
                + (f.vorp != null || o.vorp != null ? statRow('VORP', f.vorp, o.vorp, true, function (x) {
                    return x != null ? fmtSigned(x, Number.isInteger(x) ? 0 : 1) : 'N/A';
                }) : '')
                + statRow('ADP', f.adp, o.adp, false, function (x) {
                    return x != null ? Number(x).toFixed(1) : 'N/A';
                })
                + statRow('vs ADP', f.vsAdp, o.vsAdp, true, function (x) {
                    return fmtSigned(Math.round(x), 0);
                })
                ...
                + (state.type !== 'redraft' ? statRow('Tier', f.tier, o.tier, false, function (x) {
                    return x != null ? 'T' + x : '-';
                }) : '')
                + (state.type !== 'redraft' ? statRow('Age', f.age, o.age, false, function (x) {
                    return x != null ? x.toFixed(0) : '-';
                }) : '')
                + (f.bye != null || o.bye != null ? statRow('Bye', f.bye, o.bye, false, function (x) {
                    return x != null ? String(x) : '-';
                }) : '')
                + (f.rec != null || o.rec != null ? statRow('REC', f.rec, o.rec, false, function (x) {
                    return x != null ? '#' + x : '-';
                }, 'Recommendation Rank — who to draft now (roster-aware order, not a grade)') : '')
                + (f.survive != null || o.survive != null ? statRow('Survive', f.survive, o.survive, true, function (x) {
                    return x != null ? x + '%' : '-';
                }) : '')
                + (f.projPts != null || o.projPts != null ? statRow('Proj Pts', f.projPts, o.projPts, true, function (x) {
                    return x != null ? String(Math.round(x)) : '-';
                }) : '')
                + (f.market != null || o.market != null ? statRow('Mkt vs ADP', f.market, o.market, true, function (x) {
                    return fmtSigned(Math.round(x), 0);
                }) : '')
                + '</div></div>';
        }
    ...
    function prepareNextPickValues(pool) {
        var c = psCtx(), next = recWaitPickNo();
        c.nextByPos = {};
        c.demandByPos = {};
        if (!next) return;
        ...
        ['QB', 'RB', 'WR', 'TE'].forEach(function (pos) {
            var rows = pool.filter(function (p) {
                return String(p.position || '').toUpperCase() === pos && p._ps != null;
            })
                .sort(function (a, b) {
                    return b._ps - a._ps;
                }).slice(0, 16);
    ...
    function liveDecisionScore(p, counts) {
        ...
        var score = DraftBoardCore.decisionScore({
    ...
    function isQueued(id) {
        return !!(state.queue && state.queue.indexOf(String(id)) >= 0);
    }
    ...
    function renderQueue() {
        var q = (state.queue || []).map(function (id) {
            return playersById[String(id)];
        })
            .filter(function (p) {
                return p && !drafted[String(p.id)];
            });
        if (!q.length) {
            listInto(emptyNote('Queue is empty', 'Tap the ★ on any player to add a target.'));
            return;
        }
    ...
    function renderSide() {
        ...
        for (var i = 0; i < kbtns.length; i++) {
            kbtns[i].style.display = kdef ? '' : 'none';
        }
    ...
    function showCompleteSidebar() {
        ...
        try {
            refreshServerPlayoffOdds(gradeAllTeams());
        } catch (e) { /* grades may not be ready yet */
        }
    }
    ...
    function runBanner() {
        ...
        for (var pn = state.current - 1; pn >= 1 && n < 5; pn--) {
            var p = state.picks[pn];
            if (!p) continue;
            var pos = (p.position || '').toUpperCase();
            if (last5[pos] != null) last5[pos]++;
            n++;
        }
        var hot = '';
        ['RB', 'WR', 'QB', 'TE'].forEach(function (pos) {
            if (!hot && last5[pos] >= 3) hot = pos;
        });
        if (!hot) return '';
    ...
    function alertBanners() {
        return runBanner() + cliffBanner();
    }

    function renderRec() {
        if (!hasOwned()) {
            listInto(emptyNote('Set your pick slot', 'Choose your draft slot to get personalized recommendations.'));
            return;
        }
        var counts = myPosCounts();
        var pool = availablePool().slice();
        if (!pool.length) {
            listInto(emptyNote('No players available', 'Everyone matching this filter has already been drafted.', _DR_SEARCH_ICON));
            return;
        }
        var maxVal = 0;
        pool.forEach(function (p) {
        ...
        pool.forEach(function (p) {
            p._ps = pickScore(p, maxVal, counts);
        });
        prepareNextPickValues(pool);
        pool.forEach(function (p) {
            p._ds = liveDecisionScore(p, counts);
        ...
        pool.sort(function (a, b) {
            return (b._ds || 0) - (a._ds || 0) || (b._ps || 0) - (a._ps || 0);
        });
    ...
    function lineupScore(p) {
        ...
        var ppg = ppgOf(p);
        if (ppg == null) {
            var full = playersById[String(p.id)];
            if (full) ppg = ppgOf(full);
        }
        if (ppg != null) return Number(ppg);
        var v = (p.val != null) ? Number(p.val) : null;
        if (v == null) {
            var f2 = playersById[String(p.id)];
            if (f2) v = valOf(f2);
        }
        return (v || 0) / 1000;
    }
    ...
    function optimalLineup(playerList, slots) {
        ...
        order.sort(function (a, b) {
            return (flex[a.slot] || 1) - (flex[b.slot] || 1) || a.i - b.i;
        });
        var used = {}, assign = {};
        ...
        var starters = slots.map(function (s, i) {
            return {slot: s, p: assign[i] || null};
        });
        var bench = [];
        ...
        starters.forEach(function (s) {
            if (s.p) starterIds[String(s.p.id)] = true;
        });
    ...
    function playoffOddsBySlot(allTeams) {
        var sig = allTeams.map(function (t) {
            return t.slot + ':' + (t.picks ? t.picks.length : 0);
        }).join('|') + '@' + state.current;
        if (_poMcCache && _poMcSig === sig) return _poMcCache;
        var teams = allTeams.map(function (t) {
            var picks = (t.picks || []).map(function (x) {
                return x.p;
            }).filter(Boolean);
        ...
        if (n >= 2) {
            ...
            for (var s = 0; s < N; s++) {
                ...
                var ord = [];
                for (var o = 0; o < n; o++) ord[o] = o;
                ord.sort(function (x, y) {
                    return (wins[y] - wins[x]) || (pts[y] - pts[x]);
                });
                for (var r = 0; r < spots; r++) {
                    odds[teams[ord[r]].slot] += 1;
                }
            }
            teams.forEach(function (t) {
                odds[t.slot] = Math.round(odds[t.slot] / N * 100);
            });
        }
        _poMcCache = odds;
        _poMcSig = sig;
    ...
    function _poSig(allTeams) {
        return allTeams.map(function (t) {
            return t.slot + ':' + (t.picks ? t.picks.length : 0);
        }).join('|') + '@' + state.current;
    }
    ...
    function _repaintPlayoffOdds() {
        ...
        if (sum && sum.style.display !== 'none' && typeof openSummary === 'function') {
            try {
                openSummary();
            } catch (e) { /* openSummary may not be ready */
            }
        }
    ...
    function refreshServerPlayoffOdds(allTeams) {
        ...
        if (youTeam && viewerSeat && !teamsPayload.some(function (t) {
            return Number(t.slot) === Number(viewerSeat);
        })) {
            teamsPayload.push({
                slot: viewerSeat,
                roster_id: viewerRid,
        ...
        fetch('/api/draft-playoff-odds', {
            ...
            .then(function (resp) {
                ...
                if (resp && resp.odds && resp.odds.length) {
                    var m = {};
                    resp.odds.forEach(function (o) {
                        if (o.slot != null) m[o.slot] = o.playoff_pct;
                    });
    ...
    function gradeTeam() {
        ...
        for (var i = 0; i < field.length; i++) {
            if (field[i].isMe) return field[i].grade;
        }
        var mine = [];
        Object.keys(state.picks).forEach(function (k) {
            var pn = parseInt(k, 10);
        ...
        mine.sort(function (a, b) {
            return a.pn - b.pn;
        }); // process in pick order for need context
    ...
    function teamLetterFromPicks(letters) {
        ...
        var avg = letters.reduce(function (s, g) {
            return s + (gv[g] != null ? gv[g] : 2);
        }, 0) / letters.length;
        if (avg >= 4.5) return 'A+';
        if (avg >= 3.5) return 'A';
        if (avg >= 2.5) return 'B';
    ...
    function gradeRowsForPicks(mine) {
        ...
        (mine || []).forEach(function (m) {
            ...
            picks.push({
                id: m.p.id, pos: pos, ps: ps, pn: m.pn, keeper: kept,
    ...
    function gradePicks(mine) {
        ...
        var relVals = mine.filter(function (m) {
            return !isKeeperPick(m.p);
        })
            .map(function (m) {
                return relPS(m.p, m.pn);
            })
            .filter(function (v) {
                return v != null;
            });
        var avgPs = relVals.length
            ? relVals.reduce(function (a, b) {
            return a + b;
        }, 0) / relVals.length
            : null;
        ...
        if (state.type === 'rookie') {
            ...
            var _rk = _letters.filter(function (L) {
                return L && L !== 'N/A';
            })
                .map(function (L) {
            ...
            var rv = _rk.reduce(function (a, b) {
                return a + b;
            }, 0) / _rk.length;
            return {
                score: rv, value: avgPs != null ? Math.round(avgPs) : 50,
        ...
        var _slots = lineupSlots().filter(function (s) {
            return s !== 'K' && s !== 'DEF';
        });
        var _leaguePpg = [];
        players.forEach(function (q) {
            var v = ppgOf(q);
            if (v != null) _leaguePpg.push(v);
        });
        var _leagueVal = players.map(function (q) {
            return valOf(q);
        });
        ...
        var _starterArr = picks.filter(function (x) {
            return _comp.starterIds[String(x.id)];
        });
        return {
            score: _comp.total, value: _comp.value, balance: _comp.balance, tier: _comp.starter,
    ...
    function _buildGradeCliffs() {
        ...
        var teams = (state && state.teams) || 12;
        Object.keys(state.picks || {}).filter(function (k) {
            return !!state.picks[k];
        })
            .map(function (k) {
                return parseInt(k, 10);
    ...
    function _applyFieldCurve(out) {
        ...
        out.forEach(function (t) {
            if (t && t.grade) t.grade.rawScore = t.grade.score;
        });
    }
    ...
    function renderNeeds() {
        if (!hasOwned()) {
            listInto(emptyNote('Set your pick slot', 'Choose your draft slot to see your team build.'));
            return;
        }
        var mine = myPicksList().slice().sort(function (a, b) {
        ...
        if (g) {
            ...
            var gSub = '';
            var _ga = teamArchetype();
            if (_ga) gSub = _ga.label;
            var _gwn = g.window;
        ...
        function _pPpg(p) {
            return scoringProjPpg(p);
        }

        var projPlayers = mine.filter(function (p) {
            return _pPpg(p) != null;
        });
        if (projPlayers.length >= 2) {
            var myProjTotal = 0;
            projPlayers.forEach(function (p) {
                myProjTotal += _pPpg(p);
            });
            ...
            var leagueAvgPerPlayer = topSlice.length ? topSlice.reduce(function (a, b) {
                return a + b;
            }, 0) / topSlice.length : 0;
    ...
    var _DR_METALS = {
        gold: {
            face: ['#fff7d6', '#f6d375', '#d9a531', '#a9781f'],
            rim: ['#ffe89a', '#c99a2e', '#7d5a12'],
            edge: '#7d5a12',
            num: '#8a6410'
        },
        silver: {
            face: ['#ffffff', '#dde3ea', '#a7b2be', '#727d89'],
            rim: ['#eef2f6', '#aab4bf', '#66707b'],
            edge: '#66707b',
            num: '#5c6670'
        },
        bronze: {
            face: ['#ffe6cc', '#e0a56a', '#bd7539', '#7f4a22'],
            rim: ['#f0c39a', '#b87a44', '#7a4620'],
            edge: '#7a4620',
            num: '#7a4620'
        }
    };
    ...
    function _draftRecapHtml(allTeams) {
        ...
        var withGap = picks.filter(function (p) {
            return p.gap != null;
        });
        ...
        function rx(pn) {
            ...
            var pp = pn - (rd - 1) * teams;
            return rd + '.' + (pp < 10 ? '0' + pp : pp);
        }

        function valTxt(x) {
            return (useGap && x.gap != null) ? ((x.gap > 0 ? '+' : '') + x.gap) : String(x.ps);
        }
        ...
        var steals = pool.slice().sort(function (a, b) {
            return useGap ? (b.gap - a.gap) : (b.ps - a.ps);
        }).slice(0, 4);
        var reaches = pool.slice().sort(function (a, b) {
            return useGap ? (a.gap - b.gap) : (a.ps - b.ps);
        }).slice(0, 4);

        // ── By the numbers: a few whole-draft superlatives ──────────────────
        var teamScores = {};
        picks.forEach(function (p) {
            (teamScores[p.team] = teamScores[p.team] || []).push(p.ps);
        });
        ...
        var posCount = {};
        picks.forEach(function (p) {
            if (p.pos) posCount[p.pos] = (posCount[p.pos] || 0) + 1;
        });
        var topPos = Object.keys(posCount).sort(function (a, b) {
            return posCount[b] - posCount[a];
        })[0] || '-';
        ...
        document.querySelectorAll('#drBaList [data-legslot]').forEach(function (row) {
            row.addEventListener('click', function () {
                ...
                if (isOpen && !dtl.innerHTML) {
                    var team = allTeams.filter(function (t) {
                        return t.slot === slot;
                    })[0];
                    if (!team || !team.picks) return;
                    var _sp = team.picks.slice().map(function (x) {
                        return x.p;
                    }).filter(Boolean);
                    var _tst = optimalLineup(_sp).starters.filter(function (x) {
                        return x.p;
                    });
                    ...
                    _tst.forEach(function (x) {
                        if (x.p && x.p.id != null) _starterIds[x.p.id] = true;
                    });
                    var _bench = team.picks.slice()
                        .filter(function (pk) {
                            return pk.p && !_starterIds[pk.p.id];
                        })
                        .sort(function (a, b) {
                            return (a.pn || 0) - (b.pn || 0);
                        });

                    function _pickRx(pn) {
                        if (!pn) return '';
                        var rd = Math.ceil(pn / state.teams);
                        var pp = pn - (rd - 1) * state.teams;
                        return rd + '.' + (pp < 10 ? '0' + pp : pp);
                    }
                    ...
                    var dtlHtml = '';
                    _tst.forEach(function (x) {
                        var _pnx = (team.picks.filter(function (pk) {
                            return pk.p && pk.p.id === x.p.id;
                        })[0] || {}).pn || 0;
                        dtlHtml += _ldtlRow(x.slot, x.p, _pnx);
                    });
                    _bench.forEach(function (pk) {
                        dtlHtml += _ldtlRow('BN', pk.p, pk.pn || 0);
                    });
    ...
    function valLookup(id) {
        var p = playersById[String(id)];
        return (p && state) ? Math.round(valOf(p)) : null;
    }
    ...
    function applyOneLivePick(p, cap) {
        ...
        if (row) {
            try {
                pickObj.ps = pickScoreFor(row);
            } catch (e) {
            }
            try {
                pickObj.psRel = psDisplay(pickObj.ps);
            } catch (e) {
            }
        }
    ...
    function detectLive() {
        ...
        box.style.display = '';
        box.innerHTML = '<div class="dr-live-head">Detecting drafts…</div>';
        ...
        fetch(detectUrl)
            ...
            .then(function (resp) {
                if (resp.unsupported) {
                    box.innerHTML = '<div class="dr-live-head">Live sync currently supports Sleeper, ESPN, and Yahoo leagues.</div>';
                    return;
                }
                ...
                var all = resp.drafts || [];
                if (!all.length) {
                    box.innerHTML = '<div class="dr-live-head">No drafts found for this league yet.</div>';
                    return;
                }
                ...
                var active = all.filter(function (d) {
                    return String(d.status) !== 'complete';
                });
                ...
                ds.forEach(function (d) {
                    ...
                    if (d.start_time) {
                        try {
                            ...
                            when = ' · ' + (sameDay ? t : (dt.toLocaleDateString([], {
                                month: 'short',
                                day: 'numeric'
                            }) + ' ' + t));
                        } catch (e) {
            ...
            .catch(function () {
                box.innerHTML = '<div class="dr-live-head">Could not detect drafts.</div>';
            });
    }
    ...
    function buildPickOwnersFromResponse(d, teams, rounds, order) {
        ...
        if (d.draft_order && d.user_roster_map) {
            Object.keys(d.draft_order).forEach(function (uid) {
                var sl = d.draft_order[uid], rid = d.user_roster_map[uid];
                if (sl != null && rid != null) {
                    slotToRosterId[sl] = rid;
                    rosterToSlot[rid] = parseInt(sl, 10);
                }
            });
        }
    ...
    function _connectLiveApply(d, draftId) {
        {
            ...
            _espnStallPolls = 0;
            _espnEverGrew = liveSelectionCount(d.picks) > 0;
            _espnLastPickCount = liveSelectionCount(d.picks);
            _espnAuthFailed = false;
            _espnFallbackShown = false;
            _espnRelayActive = false;
            _espnRelayLastFp = '';
            hideEspnFallback();
    ...
    function updateEspnSyncPill(kind) {
        ...
        if (kind === 'connecting') {
            label = who + ' Draft · Connecting';
            cls += ' is-warn';
        } else if (kind === 'not_started') {
            label = who + ' Draft · Not Started';
            cls += ' is-muted';
        } else if (kind === 'live') {
            ...
            label = who + ' Draft · Synced';
            cls += ' is-ok';
        } else if (kind === 'unavailable') {
            label = who + ' Draft · Sync Unavailable';
            cls += ' is-warn';
        } else if (kind === 'complete') {
            label = who + ' Draft · Complete';
            cls += ' is-ok';
        }
        el.className = cls;
    ...
    function openEspnExtensionInstall() {
        var storeUrl = String(cfg.chromeExtensionStoreUrl || '').trim();
        if (storeUrl) {
            window.open(storeUrl, '_blank', 'noopener');
            return;
        }
        var zipUrl = String(cfg.chromeExtensionZipUrl || '/static/extension/br-fantasy-espn-connector.zip');
        ...
        dl.className = 'dr-btn dr-btn-primary';
        dl.href = zipUrl;
    ...
    function _wireEspnExtensionRelay() {
        ...
        _listenBrFantasy('brfantasy:espn-draft-relay', function (ev) {
            _espnExtLastSeen = Date.now();
            try {
                applyEspnExtensionRelay(ev && ev.detail);
            } catch (err) {
                if (window.console) console.error('[draft] espn relay', err);
            }
        });
        _listenBrFantasy('brfantasy:yahoo-draft-relay', function (ev) {
            _espnExtLastSeen = Date.now();
            try {
                applyYahooExtensionRelay(ev && ev.detail);
            } catch (err) {
                if (window.console) console.error('[draft] yahoo relay', err);
            }
        });
        _listenBrFantasy('brfantasy:extension-reconnect', function () {
            try {
                reconnectExtensionSync(true);
            } catch (err) {
                if (window.console) console.error('[draft] extension reconnect', err);
            }
        });
            ...
            .then(function (res) {
                ...
                if (sig !== _liveSig) {
                    ...
                    (d.picks || []).forEach(function (pk) {
                        if (pk.picked_at && pk.picked_at > _newPickedAt) _newPickedAt = pk.picked_at;
                    });
                    if (_newPickedAt) {
    ...
    function stopPolling() {
        ...
        var sb = document.getElementById('drStartBanner');
    ...
    function renderLeagueMeta() {
        ...
        el.innerHTML = parts.map(function (p) {
            return '<span class="dr-lm-chip">' + p + '</span>';
        }).join('');
    ...
    function renderStatus() {
        ...
        if (done) {
            oc.textContent = 'Draft complete';
            if (ocLabel) ocLabel.style.display = 'none';
        } else if (notStarted) {
            oc.textContent = 'Draft hasn\'t started';
            if (ocLabel) ocLabel.style.display = 'none';
        } else if (sim && !simStarted) {
            oc.textContent = 'Ready to draft';
            if (ocLabel) {
                ocLabel.style.display = '';
                ocLabel.textContent = 'Claim picks, then Start';
            }
        } else {
            var slot = slotOnClock(state.current, state.teams, state.order);
            ...
            if (ocLabel) {
                ocLabel.style.display = '';
                ocLabel.textContent = 'On the clock';
            }
        }
        ...
        if (g) {
            gp.style.display = '';
            gp.textContent = 'Grade ' + gradeLetter(g.score) + gradeEarlySuffix(g);
        } else {
        ...
        var _anyPicks = !!(state && state.picks && Object.keys(state.picks).some(function (k) {
            return !!state.picks[k];
        }));
    ...
    function boardSig() {
        return [state.teams, state.rounds, state.order, state.slot, state.mode || 'm', ownedPicks().join(',')].join('|');
    }
    ...
    function syncAdpSourceSelector() {
        ...
        if (!ui) {
            ...
            menu.addEventListener('click', function (e) {
                var opt = e.target.closest('.dr-sortsel-opt');
                if (!opt) return;
                e.stopPropagation();
                menu.hidden = true;
    ...
    function renderBA() {
        ...
        if (!pool.length) {
            listInto(emptyNote('No players match', 'Try another position filter or clear your search.', _DR_SEARCH_ICON));
            return;
        }
        ...
        var _isKD = function (p) {
            var pos = String(p.position || '').toUpperCase();
            return pos === 'K' || pos === 'DEF';
        };
        var mainPool = (_posIsAll() && wantsKDef()) ? pool.filter(function (p) {
            return !_isKD(p);
        }) : pool;
        ...
        if (_posIsAll() && wantsKDef() && kdPool.length && hasOwned()) {
            ...
            if (_kdefTime) {
                if (_needK > 0) {
                    var _bk = kdPool.filter(function (p) {
                        return String(p.position).toUpperCase() === 'K';
                    })[0];
                    if (_bk) _promoted.push(_bk);
                }
                if (_needDef > 0) {
                    var _bd = kdPool.filter(function (p) {
                        return String(p.position).toUpperCase() === 'DEF';
                    })[0];
                    if (_bd) _promoted.push(_bd);
                }
                kdPool = kdPool.filter(function (p) {
                    return _promoted.indexOf(p) < 0;
                });
            }
        }
    ...
    var _GLOSSARY = [
        ...
        {
            term: 'Draft Grade',
            def: 'How good is the resulting roster? It primarily evaluates the optimal starters, functional bench depth, efficient construction, and role- and round-weighted pick quality. It is not an average Recommendation Rank or Decision Score, and K/DEF are grade-neutral.'
        },
        {
            term: 'Value',
            def: 'The player’s trade value as an asset on a 0-999 scale - dynasty value for startup/rookie drafts, redraft value for redraft.'
        },
        {
            term: 'VOR / VORP',
            def: 'Value Over Replacement: how much better a player is than a replacement-level starter at their position (a fixed, preseason-style baseline). VORP uses projected season fantasy points; VOR uses dynasty or redraft trade value. Last season\'s injury-shortened totals are not used.'
        },
        {
            term: 'ADP',
            def: 'Average Draft Position - the typical overall pick a player goes at in real drafts. If it’s below your current pick, they’ve fallen and may be a value. When a sample size (n=) is shown, a small n means the ADP is noisy.'
        },
        {
            term: 'Tier',
            def: 'Players grouped by talent gaps (Tier 1 = elite). A tier “cliff” means only a couple of players remain before a real drop-off at that position.'
        },
        {
            term: 'PPG',
            def: 'Points per game - projected for the upcoming season, or last season’s actual when that’s shown. On the draft report card, Proj PPG is the projected weekly total of your optimal starting lineup, not the sum of every player you drafted.'
        },
        {
            term: 'Survival %',
            def: 'The chance a player is still on the board at your next pick. Starts from consensus ADP, then adapts to how your draft is actually going - if the room is reaching, letting players slide, drafting unpredictably, or running on a position, the odds shift to match (kicks in after the first several picks).'
        },
        {
            term: 'Grade · Value',
            def: 'How strong your picks are by pick score versus this league’s average, weighted toward the earlier rounds where it matters most.'
        },
        {
            term: 'Grade · Starters',
            def: 'How good your projected starting lineup is versus this league’s actual starting lineups. 100% is the average of those lineups; the rank is among teams in this draft. Snake drafts are close to zero-sum, so a lineup near 100% of average can still rank 1st or 2nd.'
        },
        {term: 'Grade · Construction', def: 'How well you’ve filled your starting slots and balanced your positions.'},
        {
            term: 'Grade · Early',
            def: 'Shown until your team has 8 picks (3 in a rookie draft). The letter is real — including at two picks / the start of round 3 — but construction is still ramping and the sample is small.'
        },
        {
            term: 'Hist',
            def: 'Historical top-12 chance for this career profile and situation. Compare it to the ADP-round rate in Deep Dive — early ADP is a high bar. Not a Pick Score, Recommendation Rank, VOR, or Draft Grade input.'
        }
    ];
    ...
    function closeGlossary() {
        document.getElementById('drGloss').style.display = 'none';
    }

    // ── Summary overlay ─────────────────────────────────────────────────────────
    function openSummary() {
        var hasSlot = hasOwned();
        var hasPicks = state && Object.keys(state.picks || {}).some(function (k) {
            return !!state.picks[k];
        });
        ...
        if (hasSlot) {
            ...
            var _idToPn = {};
            Object.keys(state.picks || {}).forEach(function (k) {
                var _pp = state.picks[k];
                if (_pp) _idToPn[String(_pp.id)] = parseInt(k, 10);
            });
        ...
        if (hasSlot && mine.length) {
            ...
            mine.forEach(function (p) {
                ...
                var _t = tierOf(_fp);
                if (_t != null && _t <= 2) sumT12++;
                var _psShown = relPS(p);
                if (_psShown != null) {
                    sumAllPsTotal += _psShown;
                    sumAllPsCount++;
                }
                if (_ssSet[String(p.id)] && _psShown != null) {
                    sumStarterPsTotal += _psShown;
                    sumStarterPsCount++;
                }
            });
            var _sits = [];
            ...
            if (state.type === 'rookie') {
                ...
                if (sumStarterPsCount >= 2) _sits.push({
                    v: Math.round(sumStarterPsTotal / sumStarterPsCount),
                    l: 'Starter PS'
                });
            }
            ...
            if (_draftComplete()) {
                ...
                for (var _ti = 0; _ti < _allT.length; _ti++) {
                    if (_allT[_ti].isMe) {
                        _meT = _allT[_ti];
                        break;
                    }
            ...
            if (_sits.length) {
                ...
                _sits.forEach(function (s) {
                    statsHtml += '<div class="dr-sum-stat"><div class="dr-sum-stat-v">' + s.v + '</div><div class="dr-sum-stat-l">' + s.l + '</div></div>';
                });
                statsHtml += '</div>';
            }
        }
        ...
        function sumRow(slot, p) {
            ...
            var _pn = (Object.keys(state.picks).filter(function (k) {
                return state.picks[k] && state.picks[k].id === p.id;
            }).map(function (k) {
                return parseInt(k, 10);
            })[0]) || 0;
            var pickStr = _pn ? (function () {
                var _rd = Math.ceil(_pn / state.teams);
                var _pp = _pn - (_rd - 1) * state.teams;
                return 'Pick ' + _rd + '.' + (_pp < 10 ? '0' + _pp : String(_pp));
            })() : '';
        ...
        if (hasSlot) {
            ...
            if (bench.length) {
                bench.forEach(function (p) {
                    starterBenchHtml += sumRow('BN', p);
                });
            } else {
                starterBenchHtml += sumRow('BN', null);
            }
        }
        ...
        if (_ddBtn) _ddBtn.addEventListener('click', function () {
            closeSummary();
            openDeepDive();
        });
        var _shareBtn = document.getElementById('drSumShare');
        if (_shareBtn) _shareBtn.addEventListener('click', function () {
            closeSummary();
            shareDraft();
        });
    }
    ...
    function deepDiveAdp(p) {
        return p && p.consAdp != null ? p.consAdp : (p ? p.adp : null);
    }

    function deepDiveDiff(p) {
        return p && p.consDiff != null ? p.consDiff : (p ? p.diff : null);
    }
    ...
    function ddVerdict(p) {
        ...
        if (Core && Core.adpDeltaVerdict) {
            var verdict = Core.adpDeltaVerdict({
                diff: diff, isBpa: bpa, survivePct: p.survivePct,
    ...
    function ddHistoricalAlternatives(pn, selected, taken) {
        var Core = window.DraftBoardCore;
        if (!Core || !Core.decisionScore) return null;
        ...
        var ctx = Core.historicalDecisionContext({
            ...
            qualityOf: function (f) {
                var q = ppgNormOf(f);
                return q == null ? 0.35 : q;
            },
            vorPositive: function (f) {
                return vorOf(f) == null || vorOf(f) > 0;
            }
        });
        ...
        remaining.forEach(function (p) {
            maxV = Math.max(maxV, valOf(p));
        });
        var rs = (state && state.roster) || defaultRoster(),
            opts = {sf: !!state.sf, tep: scoringCfg().tep, draftType: state.type};
        var rd = ctx.round;
        var expectedByPos = {};
        remaining.forEach(function (p) {
            var pos = String(p.position || '').toUpperCase();
        ...
        Object.keys(expectedByPos).forEach(function (pos) {
            expectedByPos[pos].sort(function (a, b) {
                return b - a;
            });
        });
        ...
        return {
            ...
            topAlternatives: (summary.topAlternatives || []).map(function (r) {
                return r.player;
            })
        };
    }
    ...
    function ddMyPicks() {
        ...
        Object.keys(state.picks).forEach(function (k) {
            ...
            var consAdpFn = function (p) {
                var c = consensusAdpOf(p);
                return c != null ? c : adpOf(p);
            };
            ...
            Object.keys(full.adp_by_source || {}).forEach(function (src) {
                var v = adpBySource(full, src);
                if (v != null) sourceValues.push(v);
            });
            ...
            rows.push({
                pn: pn, pl: pl, full: full, pos: pos, keeper: isKeeperPick(pl),
    ...
    function ddRankBy(field, keyFn) {
        var arr = field.map(function (t) {
            return {slot: t.slot, v: keyFn(t.grade) || 0};
        })
            .sort(function (a, b) {
                return b.v - a.v;
            });
    ...
    function ddPosRankIndex() {
        ...
        Object.keys(byPos).forEach(function (pos) {
            byPos[pos].sort(function (a, b) {
                return (b.ppg - a.ppg) || (b.val - a.val);
            });
        });
        return byPos;
    }
    ...
    function openDeepDive() {
        ...
        var openPicks = picks.filter(function (p) {
            return !p.keeper;
        });
        var withAdp = openPicks.filter(function (p) {
            return deepDiveDiff(p) != null;
        });
        ...
        var nValues = withAdp.filter(function (p) {
            return deepDiveDiff(p) >= 3;
        }).length;
        var nReaches = withAdp.filter(function (p) {
            return ddVerdict(p).cls === 'reach';
        }).length;
    ...
    function closeDeepDive() {
        var o = document.getElementById('drDeepDive');
        if (o) o.style.display = 'none';
        var t = document.getElementById('drDdTip');
    ...
    function ddOverviewHtml(me, myRank, n, field, netValue, nValues, nReaches) {
        ...
        var m = gradeMax();
        var vRank = ddRankBy(field, function (x) {
            return x.value;
        })[me.slot];
        ...
        var sRank = ddRankBy(field, function (x) {
            return x.strength != null ? x.strength : x.tier;
        })[me.slot];
        var cRank = ddRankBy(field, function (x) {
            return x.balance;
        })[me.slot];
    ...
    function ddOverviewVerdict(g, myRank, n, netValue, arch) {
        ...
        comps.forEach(function (c) {
            if (c.pct >= 0.72) strong.push(c.k); else if (c.pct <= 0.5) weak.push(c.k);
        });
    ...
    function ddTlAdp(p) {
        return deepDiveAdp(p);
    }

    function ddTimelineHtml(picks) {
        var hasCons = picks.some(function (p) {
            return p.consAdp != null;
        });
        ...
        return '<div class="dd-card">'
            ...
            + ['QB', 'RB', 'WR', 'TE'].map(function (p) {
                return '<span><i class="dd-dot" style="background:' + posColor(p) + '"></i>' + p + '</span>';
            }).join('')
    ...
    function ddDrawTimeline(picks) {
        ...
        function el(nm, a) {
            var e = document.createElementNS(NS, nm);
        ...
        maxD = Math.ceil(maxD / 5) * 5 + 2;
        minD = Math.floor(minD / 5) * 5 - 2;
        var x = function (i) {
            return mr.l + (pts.length === 1 ? iw / 2 : (i / (pts.length - 1)) * iw);
        };
        var y = function (d) {
            return mr.t + (maxD - d) / (maxD - minD) * ih;
        };
        var y0 = y(0);
        ...
        svg.appendChild(el('rect', {
            ...
            width: iw,
            height: mr.t + ih - y0,
            fill: 'color-mix(in srgb,#ef4444 7%,transparent)'
        }));
        ...
        pts.forEach(function (p) {
            cum += ddTlDelta(p);
            cpts.push(cum);
            if (Math.abs(cum) > cmax) cmax = Math.abs(cum);
        });
        var cy = function (v) {
            return mr.t + ih / 2 - (v / cmax) * (ih / 2) * 0.9;
        };
        var dpath = cpts.map(function (v, i) {
            return (i ? 'L' : 'M') + x(i).toFixed(1) + ' ' + cy(v).toFixed(1);
        }).join(' ');
        ...
        pts.forEach(function (p, i) {
            ...
            var dot = el('circle', {
                ...
                tabindex: '0',
                role: 'button',
                'aria-label': p.pl.name + ', pick ' + roundPickStr(p.pn) + ', ' + fmtAdpDelta(ddTlDelta(p)) + ' versus ADP'
            });
            ...
            dot.addEventListener('focus', function () {
                ...
                ddTip({clientX: box.left + box.width / 2, clientY: box.top}, p);
            });
            ...
            var rl = el('text', {
                ...
                y: H - mr.b + 19,
                'text-anchor': 'middle',
                'font-size': compact ? 11 : 9,
                fill: 'var(--text-subtle,var(--text-muted))'
            });
            rl.textContent = roundPickStr(p.pn);
            svg.appendChild(rl);
        });
    }
    ...
    function ddWireLedger(picks) {
        var body = document.getElementById('drDdLedgerBody');
        if (!body) return;
        var st = {k: 'diff', dir: -1};
        ...
        ths.forEach(function (th) {
            th.addEventListener('click', function () {
                ...
                st.dir = (st.k === k) ? -st.dir : (t === 'n' ? -1 : 1);
                st.k = k;
                var list = picks.slice().sort(function (a, b) {
                    var av, bv;
                    if (k === 'vord') {
                        av = ddVerdict(a).label;
                        bv = ddVerdict(b).label;
                        return st.dir * String(av).localeCompare(String(bv));
                    }
                    if (k === 'name') {
                        return st.dir * String(a.pl.name).localeCompare(String(b.pl.name));
                    }
                    if (k === 'pos') {
                        return st.dir * String(a.pos).localeCompare(String(b.pos));
                    }
                ...
                ths.forEach(function (o) {
                    o.classList.toggle('dd-sorted', o === th);
                });
            });
        });
    }
    ...
    function ddConstructionHtml(me, field) {
        ...
        var starterRows = ol.starters.filter(function (s) {
            return s.p && s.slot !== 'K' && s.slot !== 'DEF';
        }).map(function (s) {
    ...
    function ddHistHtml(picks) {
        ...
        var rows = (picks || []).filter(function (p) {
            return ddHistPct(p) != null;
        });
        if (!rows.length) return '';
        var avg = Math.round(rows.reduce(function (s, p) {
            return s + ddHistPct(p);
        }, 0) / rows.length);
        var above = rows.filter(function (p) {
            return ddHist(p).h_vs_m === 'history_higher';
        }).length;
        var below = rows.filter(function (p) {
            return ddHist(p).h_vs_m === 'market_higher';
        }).length;
        var edgeRows = rows.filter(function (p) {
            return ddHistVsPts(p) != null;
        });
        var avgEdge = edgeRows.length
            ? Math.round(edgeRows.reduce(function (s, p) {
                return s + ddHistVsPts(p);
            }, 0) / edgeRows.length)
            : null;
    ...
    function ddEdgesHtml(picks, me) {
        ...
        var open = (picks || []).filter(function (p) {
            return p && !p.keeper;
        });
        var withAdp = open.filter(function (p) {
            return deepDiveDiff(p) != null;
        });
        var edges = '';
        if (withAdp.length) {
            var steal = withAdp.slice().sort(function (a, b) {
                return deepDiveDiff(b) - deepDiveDiff(a);
            })[0];
            var reach = withAdp.filter(function (p) {
                return ddVerdict(p).cls === 'reach';
            })
                .sort(function (a, b) {
                    return deepDiveDiff(a) - deepDiveDiff(b);
                })[0];
            var best = open.filter(function (p) {
                return p.ps != null;
            }).sort(function (a, b) {
                return b.ps - a.ps;
            })[0];
            ...
            var stealSig = steal && window.DraftBoardCore && DraftBoardCore.significantSteal
                ? DraftBoardCore.significantSteal({
                    marketFall: deepDiveDiff(steal),
                    adpUncertainty: steal.adpTolerance,
                    boardPickScore: steal.ps
                })
        ...
        if (!cfg.isBestBall && state.type === 'redraft') {
            var benchSeen = {}, byePlayers = picks.map(function (p) {
                ...
                return {
                    ...
                    name: p.pl.name,
                    pos: p.pos,
                    bye: p.full && p.full.bye_week,
                    role: role,
                    quality: ppgNormOf(p.full) || 0
                };
            });
            ...
            if (worstBye && worstBye.level !== 'none') {
                ...
                flags.push({
                    cls: severe ? 'crit' : 'warn', ttl: 'Week ' + worstBye.week + ' — '
        ...
        ['RB', 'WR', 'TE', 'QB'].forEach(function (pos) {
            ...
            if (need > 0 && counts[pos] <= need) {
                flags.push({
                    cls: 'warn', ttl: 'Thin at ' + pos + ' — ' + counts[pos] + ' rostered',
                    ds: 'You have no margin behind your ' + pos + ' starters. Prioritize depth on the waiver wire.'
                });
            }
        });
        if (state.type === 'redraft') {
            var histBelow = picks.filter(function (p) {
                return ddHist(p).h_vs_m === 'market_higher';
            });
            if (histBelow.length >= 3) {
                flags.push({
                    cls: 'warn', ttl: histBelow.length + ' picks sit below ADP-bucket hit rates',
    ...
    function drAlert(msg, cb) {
        ...
        ok.className = 'dr-btn dr-btn-primary';
        ok.textContent = 'OK';
    ...
    function drPickTradeOpen() {
        ...
        function pickerHtml(id) {
            var ro = '';
            for (var r = 1; r <= rounds; r++) ro += '<option value="' + r + '">Round ' + r + '</option>';
            var po = '';
            for (var p = 1; p <= teams; p++) po += '<option value="' + p + '">Pick ' + p + '</option>';
        ...
        var quick = mine.length
            ...
            + mine.slice(0, 14).map(function (pn) {
                return '<button type="button" class="dr-pt-chip" data-pn="' + pn + '">' + pickLabel(pn) + '</button>';
            }).join('')
            + '</div>'
            : '';
        ...
        function renderChips() {
            [['give', 'drPtGiveChips'], ['get', 'drPtGetChips']].forEach(function (pair) {
                ...
                el.innerHTML = list.length
                    ? list.map(function (pn, i) {
                        return '<span class="dr-pt-tok">' + pickLabel(pn) + '<button type="button" class="dr-pt-tokx" data-side="' + pair[0] + '" data-i="' + i + '" aria-label="Remove">&times;</button></span>';
                    }).join('')
        ...
        msg.querySelectorAll('.dr-pt-chip').forEach(function (c) {
            c.addEventListener('click', function () {
                give.push(parseInt(this.getAttribute('data-pn'), 10));
                refresh();
            });
        });
    ...
    function commitPick(p) {
        ...
        var reason = '';
        try {
            reason = pickReason(p, myPosCounts(), {rank: recRankOf(p)});
        } catch (e) {
            reason = '';
        }
        state.picks[pn] = {
        ...
        if (state.queue) {
            var _qi = state.queue.indexOf(String(p.id));
            if (_qi >= 0) state.queue.splice(_qi, 1);
        }
        justPick = pn;
        state.current++;
        ...
        if (_revEl) {
            ...
            _revEl.addEventListener('animationend', function () {
                _revEl.classList.remove('dr-cell-reveal');
            }, {once: true});
        }
    ...
    function undo() {
        ...
        if (p) {
            delete drafted[String(p.id)];
            delete state.picks[state.current];
            invalidateKeeperAdpCache();
        }
    ...
    function resetDraft() {
        ...
        function doReset() {
            ...
            try {
                ...
                if (_u.searchParams.has('connect') || _u.searchParams.has('live')) {
                    _u.searchParams.delete('connect');
                    _u.searchParams.delete('live');
        ...
        if (isLive) {
            ...
            drConfirm('Reset the draft board? This wipes every pick and returns to setup.', 'Reset', doReset);
        }
    }
    ...
    function _readThemeVars(dark) {
        ...
        if (want !== cur) {
            want ? root.setAttribute('data-theme', want) : root.removeAttribute('data-theme');
        }
        ...
        if (want !== cur) {
            cur ? root.setAttribute('data-theme', cur) : root.removeAttribute('data-theme');
        }
        return vars;
    }
    ...
    function _buildShareCanvas(dark) {
        ...
        var rows = [];
        _olShare.starters.forEach(function (s) {
            rows.push({slot: s.slot, p: s.p});
        });
        _olShare.bench.forEach(function (p) {
            rows.push({slot: 'BN', p: p});
        });
        ...
        if (g) {
            ...
            if (!gp) {
                var _sa = teamArchetype();
                if (_sa) gp = _sa.label;
            }
            ctx.fillStyle = clr.win;
        ...
        rows.forEach(function (r, ri) {
            ...
            ctx.fillText(r.slot, bx + bw / 2, by + 15);
            ctx.textAlign = 'left';
            ...
            // Row divider
            ctx.fillStyle = clr.border;
            ctx.fillRect(pad, y + lineH - 1, W - pad * 2, 1);
            y += lineH;
        });
        return c;
    }
    ...
    function shareDraft() {
        if (!state || !hasOwned()) {
            drAlert('Pick a draft slot to build and share your team.');
            return;
        }
        // Build both themes
        ...
        tabs.forEach(function (b) {
            b.classList.toggle('is-active', b.getAttribute('data-sv') === _shareTheme);
        });
    ...
    function closePreview() {
        document.getElementById('drPreview').style.display = 'none';
    }
    ...
    document.getElementById('drSideTabs').addEventListener('click', function (e) {
        var b = e.target.closest('.otc-main-tab');
        if (!b) return;
        sideTab = b.getAttribute('data-stab');
        this.querySelectorAll('.otc-main-tab').forEach(function (x) {
    ...
    document.getElementById('drLiveList').addEventListener('click', function (e) {
        var b = e.target.closest('.dr-live-item');
        if (b) connectLive(b.getAttribute('data-id'));
    });
    ...
    function _onEspnHelperClick(e) {
        ...
        if (id === 'drEspnToolsDismiss') {
            try {
                sessionStorage.setItem(_espnToolsDismissKey(), '1');
            } catch (err) {
            }
            hideEspnTools();
        }
    ...
    if (_espnReconnectBtn) _espnReconnectBtn.addEventListener('click', function () {
        reconnectExtensionSync(false);
    });
    ...
    document.getElementById('drCellToggle').addEventListener('click', function (e) {
        var opt = e.target.closest('.dr-ct-opt');
        if (!opt) return;
        var mode = opt.getAttribute('data-mode');
        _cellShowPs = (mode === 'ps');
        this.querySelectorAll('.dr-ct-opt').forEach(function (o) {
            o.classList.toggle('is-active', o.getAttribute('data-mode') === mode);
        });
        // Repaint all filled cells so the corner stat updates immediately.
        if (state) Object.keys(state.picks).forEach(function (k) {
            if (state.picks[k]) paintCell(parseInt(k, 10));
        });
    });
    ...
    (function initOptsDropdown() {
        ...
        panel.addEventListener('click', function (e) {
            if (e.target.closest('.dr-btn')) close();
        });
    ...
    (function initSortSelect() {
        ...
        function apply(v) {
            ...
            for (var i = 0; i < opts.length; i++) {
                opts[i].classList.toggle('is-active', opts[i].getAttribute('data-val') === v);
            }
        }
        ...
        document.addEventListener('click', function (e) {
            if (isOpen() && !ui.contains(e.target)) close();
        });
        apply(cur);
    })();
    ...
    document.getElementById('drBaList').addEventListener('click', function (e) {
        ...
        if (star) {
            e.stopPropagation();
            toggleQueue(star.getAttribute('data-star'));
            return;
        }
        var draft = e.target.closest('[data-draft]');
        if (draft) {
            e.stopPropagation();
            draftPlayer(draft.getAttribute('data-draft'));
            return;
        }
    ...
    document.getElementById('drBestChips').addEventListener('click', function (e) {
        ...
        if (chip) {
            openPreview(chip.getAttribute('data-bchip'));
            return;
        }
        ...
        if (sp) {
            ...
            posFilter = {};
            posFilter[String(pos).toUpperCase()] = true;   // focus this position
            _syncPosPills();
            renderBA();
        }
    });
    ...
    document.getElementById('drCompare').addEventListener('click', function (e) {
        if (e.target === this || e.target.closest('#drCmpClose')) {
            closeCompare();
            return;
        }
        var d = e.target.closest('[data-cmp-draft]');
        if (d) {
            var id = d.getAttribute('data-cmp-draft');
            closeCompare();
            draftPlayer(id);
        }
    });
    ...
    (function () {
        var gp = document.getElementById('drGradePill');
        if (gp) gp.addEventListener('click', openSummary);
    })();
    ...
    (function () {
        ...
        if (ddc) {
            ...
            if (cfg.hasPremium) {
                var _chip = ddc.querySelector('.dr-dd-prochip');
                if (_chip) _chip.remove();
            }
        }
        var ddOv = document.getElementById('drDeepDive');
        if (ddOv) ddOv.addEventListener('click', function (e) {
    ...
    document.getElementById('drGloss').addEventListener('click', function (e) {
    ...
    document.getElementById('drShareViewClose').addEventListener('click', function () {
        document.getElementById('drShareView').style.display = 'none';
    });
    document.getElementById('drShareView').addEventListener('click', function (e) {
        if (e.target === this) this.style.display = 'none';
    });
    document.getElementById('drShareViewTabs').addEventListener('click', function (e) {
        var b = e.target.closest('.dr-shareview-tab');
        if (!b) return;
        _shareTheme = b.getAttribute('data-sv');
        this.querySelectorAll('.dr-shareview-tab').forEach(function (x) {
            x.classList.toggle('is-active', x === b);
        });
        document.getElementById('drShareViewImg').src = _shareDataUrls[_shareTheme];
    });
    document.getElementById('drShareViewShare').addEventListener('click', function () {
        _doShareOrDownload(false);
    });
    document.getElementById('drShareViewDl').addEventListener('click', function () {
        _doShareOrDownload(true);
    });
    document.getElementById('drPreview').addEventListener('click', function (e) {
        if (e.target === this || e.target.closest('#drPrevClose')) {
            closePreview();
            return;
        }
    ...
    document.getElementById('drSf').addEventListener('change', function () {
        _rosterMode = 'auto';
        _rosterPreset = null;
        _setupRoster = null;
        renderSetupRoster();
    ...
    document.getElementById('drCapitalSection').addEventListener('click', function (e) {
        ...
        if (rm) {
            delete _setupOwned[rm.getAttribute('data-rm')];
            renderSetupCapital();
            return;
        }
    ...
    function buildTeamTip(slot) {
        ...
        for (var pn = state.current; pn <= total; pn++) {
            if (ownerOf(pn) === slot && !state.picks[pn]) {
                nextPick = pn;
                break;
            }
        }
        ...
        var seatPicks = [];
        Object.keys(state.picks).map(Number).sort(function (a, b) {
            return a - b;
        }).forEach(function (pn) {
            var pick = state.picks[pn];
            if (!pick) return;
        ...
        if (state.type === 'rookie') {
            ...
            seatPicks.forEach(function (p) {
                var t = tierOf(playersById[String(p.id)] || p);
                if (t != null && t <= 2) elite++;
            });
    ...
    (function initSheet() {
        ...
        if (optsPanel && optsBtn) {
            optsPanel.addEventListener('touchstart', function (e) {
                e.stopPropagation();
            }, {passive: false});
            optsBtn.addEventListener('touchstart', function (e) {
                e.stopPropagation();
            }, {passive: false});
        }
        ...
        function applyMode() {
            if (mq.matches) {
                ...
                sheet.classList.remove('dragging');
                document.body.classList.remove('dr-sheet-expanded');
            }
        }
