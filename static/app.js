    ...
    tabs.forEach((tab) => {
        tab.addEventListener("click", () => {
            ...
            panels.forEach((p) => {
                p.classList.toggle(
                    "active",
                    p.getAttribute("data-team-id") === id
                );
            });
        });
    });
...
function initStandingsSort(root = document) {
    ...
    const applySort = () => {
        ...
        if (sortSpec.length) {
            ...
            for (let i = 1; i < sortSpec.length; i++) {
                tbl.tHead.rows[0].children[sortSpec[i].col].classList.add(
                    "sorted-secondary"
                );
            }
        }
    ...
    const toggleSort = (col, additive = false) => {
        if (!additive) {
            sortSpec = sortSpec.filter((s) => s.col === col);
        }

        const i = sortSpec.findIndex((s) => s.col === col);

        if (i === -1) {
            // first click → DESC for numeric, ASC for text
            sortSpec.push({
                col,
                dir: NUMERIC_COLS.has(col) ? -1 : 1,
            });
        } else {
...
(function () {
    ...
    refreshBtn.addEventListener("click", async () => {
        const { league, page, platform, season } = readCtxFromRefreshBtn(
            refreshBtn
        );

        if (!league || !page || !platform || !season) {
            console.error("Missing league/page/platform/season for refresh.", {
                league,
                page,
                platform,
                season,
