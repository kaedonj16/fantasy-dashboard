...
def _compute_optimal_lineup(pts_map, player_positions, roster_positions, all_pids):
    """
    Greedy optimal-lineup solver.
    Returns (set_of_optimal_starter_ids, optimal_total_pts).
    """
...
def get_model_value_table_cached():
    ...
    try:
        ...
        from utils.utils import load_players_index as _mvc_lpi, load_model_value_table as _mvc_lmvt
        draft_year = get_active_rookie_class()
        rk_rows = list(get_rookie_rankings_from_db(draft_year))
        if rk_rows:
            _mvc_idx = _mvc_lpi() or {}
            # Build calibrated value map from JSON model (same source as Prospects page).
            # This ensures rookies shown on Player Rankings use the same values as
            # everywhere else rather than the pipeline's stored rookie_value snapshot.
            _rk_mv_map: dict = {}
            try:
                _rk_json = _mvc_lmvt() or []
                _rk_mv_map = {str(p["id"]): p for p in _rk_json if p.get("id")}
            except Exception:
                pass
            ...
            for r in rk_rows:
                ...
                _mv = _rk_mv_map.get(str(sid) or "") or _rk_mv_map.get(str(_rid) or "") or {}
                tbl.append({
                    "id":        _rid,
                    "name":      name,
                    "team":      _team,
                    ...
                    "age":       r.get("age"),
                    "value":     float(_mv.get("value") or r.get("rookie_value") or 0),
                    "sf_value":  float(_mv.get("sf_value") or r.get("rookie_sf_value") or r.get("rookie_value") or 0),
                    "value_8":   float(_mv.get("value_8") or r.get("rookie_value_8") or _mv.get("value") or r.get("rookie_value") or 0),
                    "value_12":  float(_mv.get("value_12") or r.get("rookie_value_12") or _mv.get("value") or r.get("rookie_value") or 0),
                    "value_14":  float(_mv.get("value_14") or r.get("rookie_value_14") or _mv.get("value") or r.get("rookie_value") or 0),
                    "sf_value_8":  float(_mv.get("sf_value_8") or r.get("rookie_sf_value_8") or _mv.get("sf_value") or r.get("rookie_sf_value") or 0),
                    "sf_value_12": float(_mv.get("sf_value_12") or r.get("rookie_sf_value_12") or _mv.get("sf_value") or r.get("rookie_sf_value") or 0),
                    "sf_value_14": float(_mv.get("sf_value_14") or r.get("rookie_sf_value_14") or _mv.get("sf_value") or r.get("rookie_sf_value") or 0),
                    "is_rookie": True,
                })
    except Exception as e:
...
def build_portfolio_body(
    ...
    css = (
        ...
        ".pf-pos-row:first-child{border-top:none;}"
        ...
        ".nfl-abbr{min-width:34px;font-size:13px;font-weight:700;}"
        ...
        # leagues table
        ".pf-leagues-table{table-layout:fixed;width:100%;}"
        ".pf-leagues-table th,.pf-leagues-table td{vertical-align:middle;}"
        ".pf-leagues-table th:nth-child(1){width:auto;}"
        ".pf-leagues-table th:nth-child(2){width:72px;}"
        ".pf-leagues-table th:nth-child(3){width:56px;}"
    ...
    top_strip = (
        ...
        f"</div>"
        f"<div style='text-align:right;'>"
        f"<div style='font-size:11px;color:var(--text-subtle);text-transform:uppercase;letter-spacing:.06em;font-weight:600;'>Record</div>"
        f"<div style='font-size:20px;font-weight:700;' class='{rec_cls}'>{rec_str}</div>"
        f"</div>"
        f"<div style='text-align:right;'>"
        f"<div style='font-size:11px;color:var(--text-subtle);text-transform:uppercase;letter-spacing:.06em;font-weight:600;'>Season</div>"
        f"<div style='font-size:20px;font-weight:700;'>{season}</div>"
        f"</div>"
        f"</div>"
        f"</div>"
        f"</div>"
    ...
    if holdings:
        ...
        holdings_html = (
            ...
            f"<span style='font-size:13px;color:var(--text-muted);font-weight:400;'>ownership across your leagues</span>"
            f"</div>"
            f"<div class='card-body'>"
            f"<div style='display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-bottom:10px;'>"
            f"<button class='pf-pill on' data-pos='ALL'>All</button>"
            f"<button class='pf-pill' data-pos='QB'>QB</button>"
            f"<button class='pf-pill' data-pos='RB'>RB</button>"
            ...
            f"<thead><tr><th></th><th style='text-align:left;'>Player</th><th>Rank</th><th>Shares</th><th>Value</th></tr></thead>"
            f"<tbody id='pfBody'>{h_rows}</tbody>"
            f"</table>"
            f"</div>"
            f"<div style='display:flex;align-items:center;gap:8px;margin-top:10px;flex-wrap:wrap;'>"
            f"<button class='pf-pill' id='pfPrev'>&#8592; Prev</button>"
            f"<span style='font-size:13px;color:var(--text-muted);' id='pfPageInfo'></span>"
            f"<button class='pf-pill' id='pfNext'>Next &#8594;</button>"
            f"</div>"
            f"</div></div>"
            f"<script>(function(){{"
            ...
            f"if(prev)prev.disabled=pg<=1;if(next)next.disabled=pg>=pages;}}"
            f"document.querySelectorAll('.pf-pill[data-pos]').forEach(function(b){{"
            f"b.addEventListener('click',function(){{"
            f"document.querySelectorAll('.pf-pill[data-pos]').forEach(function(x){{x.classList.remove('on');}});"
