...
def build_matchup_rating_profiles(season: int, *, builder=None, logger=print) -> list[dict]:
    ...
    for label, settings in jobs:
        ...
        dedupe_key = scoring_profile_hash(settings) if settings is not None else None
        if dedupe_key is not None and dedupe_key in seen:
            logger(f"[matchup_ratings] profile hash={profile_hash} label={label} "
                   "teams=0 status=deduplicated")
            statuses.append({"hash": profile_hash, "label": label, "team_count": 0,
                             "status": "deduplicated"})
            continue
