...
def load_source_stats(season: int) -> dict[str, dict]:
    ...
    Returns player_id -> usage dict, normalized to season-total field names.

    The pre-existing usage_rows files (from the live ingestion pipeline) store
    per-game averages (avg_targets, avg_carries, avg_off_snap_pct) whereas the
    files generated by build_historical_scores use season totals (targets, carries,
    snap_share).  This function normalises both formats to the season-total form
    so downstream code can use the same field names regardless of source.
    """
    cache_path = Path("cache/player_history") / f"usage_rows_{season}.json"
    if not cache_path.exists():
        return {}
    ...
    for r in rows:
        ...
        games = max(int(u.get("games") or 0), 1)

        # Normalise per-game averages → season totals when totals are absent.
        if not u.get("targets") and u.get("avg_targets"):
            u["targets"] = round(float(u["avg_targets"]) * games)
        if not u.get("carries") and u.get("avg_carries"):
            u["carries"] = round(float(u["avg_carries"]) * games)
        if not u.get("pass_attempts") and u.get("avg_pass_att"):
            u["pass_attempts"] = round(float(u["avg_pass_att"]) * games)
        # snap_share: pre-existing files store avg_off_snap_pct as a 0-1 fraction
        if not u.get("snap_share") and u.get("avg_off_snap_pct"):
            u["snap_share"] = float(u["avg_off_snap_pct"])
