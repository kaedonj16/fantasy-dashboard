...
LEADERBOARD_METRICS: Dict[str, Dict[str, Any]] = {
    ...
    "total_pass_yards":   {"label": "Pass Yards",   "category": "Volume",    "positions": ["QB"], "integer": True, "desc": "Total passing yards in the season.", "computed_sql": "ROUND(m.yards_per_attempt * m.total_pass_att)", "computed_null": "m.yards_per_attempt IS NOT NULL AND m.total_pass_att IS NOT NULL"},
    ...
    # Total yards = rushing + receiving + passing (passing derived from ypa x att).
    "total_yards":        {"label": "Total Yards",  "category": "General", "positions": ["QB", "RB", "WR", "TE"], "integer": True, "desc": "Total yards produced in the season: rushing + receiving + passing.", "computed_sql": "ROUND(COALESCE(m.total_rush_yards, 0) + COALESCE(m.total_rec_yards, 0) + COALESCE(m.yards_per_attempt * m.total_pass_att, 0))", "computed_null": "(m.total_rush_yards IS NOT NULL OR m.total_rec_yards IS NOT NULL OR (m.yards_per_attempt IS NOT NULL AND m.total_pass_att IS NOT NULL))"},
