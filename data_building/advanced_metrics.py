            ...
            result = conn.execute("""
                ...
                r AS (
                    SELECT player_id,
                        ...
                        CASE WHEN pff_passing_grade IS NOT NULL THEN RANK() OVER (PARTITION BY (pff_passing_grade IS NULL) ORDER BY pff_passing_grade DESC) ELSE NULL END AS pff_passing_grade,
                        CASE WHEN big_time_throw_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (big_time_throw_rate IS NULL) ORDER BY big_time_throw_rate DESC) ELSE NULL END AS big_time_throw_rate,
                        CASE WHEN adjusted_completion_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (adjusted_completion_rate IS NULL) ORDER BY adjusted_completion_rate DESC) ELSE NULL END AS adjusted_completion_rate,
                        CASE WHEN nfl_passer_rating IS NOT NULL THEN RANK() OVER (PARTITION BY (nfl_passer_rating IS NULL) ORDER BY nfl_passer_rating DESC) ELSE NULL END AS nfl_passer_rating,
                        CASE WHEN yards_per_attempt IS NOT NULL THEN RANK() OVER (PARTITION BY (yards_per_attempt IS NULL) ORDER BY yards_per_attempt DESC) ELSE NULL END AS yards_per_attempt,
                        CASE WHEN completion_pct IS NOT NULL THEN RANK() OVER (PARTITION BY (completion_pct IS NULL) ORDER BY completion_pct DESC) ELSE NULL END AS completion_pct,
                        CASE WHEN td_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (td_rate IS NULL) ORDER BY td_rate DESC) ELSE NULL END AS td_rate,
                        CASE WHEN int_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (int_rate IS NULL) ORDER BY int_rate ASC) ELSE NULL END AS int_rate,
                        CASE WHEN pressure_to_sack_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (pressure_to_sack_rate IS NULL) ORDER BY pressure_to_sack_rate ASC) ELSE NULL END AS pressure_to_sack_rate,
                        CASE WHEN pff_rushing_grade IS NOT NULL THEN RANK() OVER (PARTITION BY (pff_rushing_grade IS NULL) ORDER BY pff_rushing_grade DESC) ELSE NULL END AS pff_rushing_grade,
                        CASE WHEN yards_per_carry IS NOT NULL THEN RANK() OVER (PARTITION BY (yards_per_carry IS NULL) ORDER BY yards_per_carry DESC) ELSE NULL END AS yards_per_carry,
                        CASE WHEN yards_per_touch IS NOT NULL THEN RANK() OVER (PARTITION BY (yards_per_touch IS NULL) ORDER BY yards_per_touch DESC) ELSE NULL END AS yards_per_touch,
                        CASE WHEN rush_td_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (rush_td_rate IS NULL) ORDER BY rush_td_rate DESC) ELSE NULL END AS rush_td_rate,
                        CASE WHEN elusive_rating IS NOT NULL THEN RANK() OVER (PARTITION BY (elusive_rating IS NULL) ORDER BY elusive_rating DESC) ELSE NULL END AS elusive_rating,
                        CASE WHEN breakaway_percentage IS NOT NULL THEN RANK() OVER (PARTITION BY (breakaway_percentage IS NULL) ORDER BY breakaway_percentage DESC) ELSE NULL END AS breakaway_percentage,
                        CASE WHEN explosive_runs_10_plus IS NOT NULL THEN RANK() OVER (PARTITION BY (explosive_runs_10_plus IS NULL) ORDER BY explosive_runs_10_plus DESC) ELSE NULL END AS explosive_runs_10_plus,
                        ...
                        CASE WHEN catch_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (catch_rate IS NULL) ORDER BY catch_rate DESC) ELSE NULL END AS catch_rate,
                        CASE WHEN avoided_tackles IS NOT NULL THEN RANK() OVER (PARTITION BY (avoided_tackles IS NULL) ORDER BY avoided_tackles DESC) ELSE NULL END AS avoided_tackles,
                        CASE WHEN yprr IS NOT NULL THEN RANK() OVER (PARTITION BY (yprr IS NULL) ORDER BY yprr DESC) ELSE NULL END AS yprr,
                        CASE WHEN yards_per_target IS NOT NULL THEN RANK() OVER (PARTITION BY (yards_per_target IS NULL) ORDER BY yards_per_target DESC) ELSE NULL END AS yards_per_target,
                        CASE WHEN yards_per_reception IS NOT NULL THEN RANK() OVER (PARTITION BY (yards_per_reception IS NULL) ORDER BY yards_per_reception DESC) ELSE NULL END AS yards_per_reception,
                        CASE WHEN yards_after_catch_per_reception IS NOT NULL THEN RANK() OVER (PARTITION BY (yards_after_catch_per_reception IS NULL) ORDER BY yards_after_catch_per_reception DESC) ELSE NULL END AS yards_after_catch_per_reception,
                        CASE WHEN avg_depth_of_target IS NOT NULL THEN RANK() OVER (PARTITION BY (avg_depth_of_target IS NULL) ORDER BY avg_depth_of_target DESC) ELSE NULL END AS avg_depth_of_target,
                        CASE WHEN target_share IS NOT NULL THEN RANK() OVER (PARTITION BY (target_share IS NULL) ORDER BY target_share DESC) ELSE NULL END AS target_share,
                        CASE WHEN air_yards_per_game IS NOT NULL THEN RANK() OVER (PARTITION BY (air_yards_per_game IS NULL) ORDER BY air_yards_per_game DESC) ELSE NULL END AS air_yards_per_game,
                        CASE WHEN air_yards_share IS NOT NULL THEN RANK() OVER (PARTITION BY (air_yards_share IS NULL) ORDER BY air_yards_share DESC) ELSE NULL END AS air_yards_share,
                        CASE WHEN target_quality_score IS NOT NULL THEN RANK() OVER (PARTITION BY (target_quality_score IS NULL) ORDER BY target_quality_score DESC) ELSE NULL END AS target_quality_score,
                        CASE WHEN contested_catch_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (contested_catch_rate IS NULL) ORDER BY contested_catch_rate DESC) ELSE NULL END AS contested_catch_rate,
                        CASE WHEN drop_rate IS NOT NULL THEN RANK() OVER (PARTITION BY (drop_rate IS NULL) ORDER BY drop_rate ASC) ELSE NULL END AS drop_rate,
